...
别怕，概念听起来高级，实现其实很简单。核心就一个原则：**在会话开始时把记忆内容复制一份，之后死活不动这份复制。** 下面是这个概念的示意（非真实源码，帮助理解）：

```python
# 概念演示——不是真实 Hermes 源码，展示的是"读写分离"的核心逻辑

# 会话开始：读磁盘 → 生成快照 → 冻结
frozen_memory = read_file("MEMORY.md") + read_file("USER.md")

# 每次组装 system prompt 时：永远用快照，不重读磁盘
system_prompt = SOUL.md + frozen_memory + skills_index

# Agent 中途加记忆：写磁盘，但快照不变
write_to_disk("MEMORY.md", new_content)
# frozen_memory 不动 → system prompt 不变 → 前缀缓存 OK ✅
```
...
**技术细节**：用的是 SQLite 的 FTS5 虚拟表，支持中文搜索，零外部依赖，查询速度毫秒级。

### 5.8 记忆系统的实际结构——Hermes 到底有几层记忆

Hermes 的记忆不像某些框架分五层六层，它就这三块：

| 组件 | 存什么 | 存在哪 | 
|:---|:---|:---|
| **MEMORY.md**（~2200字符） | 工作事实：项目约定、环境信息、工具怪癖 | `~/.hermes/memories/` |
| **USER.md**（~1375字符） | 用户画像：沟通偏好、工作习惯、长期目标 | `~/.hermes/memories/` |
| **session_search** | 所有历史对话记录 | SQLite FTS5 全文索引 |

可选扩展：Honcho（Plastic Labs 的用户建模引擎），需要外部服务，默认不开。

就这么简单。不用分L1-L5，那是 GA 的分法，Hermes 没这套。

### 5.9 实践落地——SOUL.md、USER.md、MEMORY.md 到底怎么写
...
### 5.11 项目级配置——AGENTS.md / CLAUDE.md（让 Agent 理解当前项目）
...
> "Hermes 的记忆就两个文件：MEMORY.md 记工作笔记（2200字上限），USER.md 记用户画像（1375字上限）。容量故意很小，逼它只记最重要的。读写分离——读的时候从冻结快照读（保护 API 缓存），写的时候直接写磁盘。聊天记录太多存不下？用 SQLite 全文搜索来翻历史。还有个可选的外部用户画像系统 Honcho（Plastic Labs 出品），默认不开启。"
