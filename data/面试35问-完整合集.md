...
| Part | 内容 | 建议轮次 |
|------|------|----------|
| **Part 0** | 项目详细介绍 · **RAG/EvoAgent 逐步附录** · 典型问题 | 开场必背 |
...
| **Part 6** | 补充新题（Q36–Q38、Q40、Q46–Q47 全流程） | 高频追问 |
...
**结论句（15 秒）**：瓶颈是 Cross-Encoder 逐条推理；批处理重排、连接池、意图路由短链路；Agentic 全链路（HyDE/re-retrieve）比裸 pipeline 慢，用 streaming + 缓存换体验。

**追问方向**：streaming 和缓存怎么设计？ · Agentic 节点怎么控延迟？
...
**题目**：OpenClaw 是 2026 年很火的开源 Agent 框架，GitHub 14 万星。你怎么看它和 EvoAgent 的设计差异？OpenClaw 的 Gateway 模式有什么优势？

---

> **轮次**：二面 · **难度**：⭐⭐⭐ · **考察点**：OpenClaw vs EvoAgent

**结论句（15 秒）**：OpenClaw 是 Gateway 触达层，EvoAgent 是轻量执行+进化系统；OpenClaw 平台化工具体系，EvoAgent 原子工具+SubAgent+对抗验证。
...
## Q28 · OpenClaw + EvoAgent 融合

**题目**：如果你要把 OpenClaw 的 Gateway 和 EvoAgent 的执行引擎融合成一个系统，你会怎么设计？

---

> **轮次**：二面 · **难度**：⭐⭐⭐⭐ · **考察点**：OpenClaw+EvoAgent 融合

**结论句（15 秒）**：Gateway→调度层→EvoAgent 执行引擎三层；松耦合、共享 L0-L4 记忆、意图路由降延迟；科研问答走 RAG 短链路。

**追问方向**：调度层怎么识别简单/复杂任务？ · Conductor inbox 怎么跨进程？
...
**题目**：Hermes Agent 是 2026 年初另一个很火的开源 Agent 框架，主打自我进化和持久记忆。你怎么看它和 EvoAgent 的异同？如果你要在一个 Agent 框架里同时实现两者的优势，你会怎么做？

---

> **轮次**：二面 · **难度**：⭐⭐⭐ · **考察点**：Hermes vs EvoAgent

**结论句（15 秒）**：共识：自主执行+分层记忆+技能结晶；Hermes 更用户友好/主动记忆/插件市场，EvoAgent 更轻+对抗验证+谨慎记忆更新。
...
**结论句（15 秒）**：成功路径固化为 L3 SOP；EvoAgent 用对抗验证+三层过滤防污染；Hermes 可能用频率阈值。
...
**结论句（15 秒）**：精力脉冲分配：Agentic RAG 为主线（2024.12 起）、EvoAgent 在 2026 集中迭代；每周一个唯一重点。
...
> **轮次**：一面 · **难度**：⭐⭐⭐⭐ · **考察点**：项目真实性 · 能否讲清完整 pipeline（与 Part 0 附录同一套答案）

**题目**：把你的 RAG 项目从上传 PDF 到返回答案，**每一步**具体怎么做？不要只说模块名，要说清输入输出和为什么。

**结论句（15 秒）**：离线七步建库（MinerU→切片→BGE-M3→Qdrant+BM25→增量 upsert）；在线 LangGraph 十二节点（意图→改写→HyDE→混合检索→门控→压缩→生成→Guard→Self-RAG）；120 问评测 + QLoRA；FastAPI SSE + React。

**追问方向**：某一步删掉指标掉多少？ · State 里有哪些字段？ · 和 Part 0 附录有何区别？
...
## Q47 · EvoAgent 全流程 · 每一步怎么做？

> **轮次**：一面/二面 · **难度**：⭐⭐⭐⭐ · **考察点**：Agent 项目真实性 · ReAct + Conductor + 记忆（与 Part 0 附录同一套答案）

**题目**：EvoAgent 从用户发消息到任务完成、记忆沉淀，**每一步**怎么做？和 LangGraph RAG 有什么区别？

**结论句（15 秒）**：MixinSession → task_queue → agent_runner_loop ReAct；L0–L4 文件记忆 + key_info；Conductor inbox 派 SubAgent；任务后 start_long_term_update；开放任务 EvoAgent，固定问答走 RAG 图。

**追问方向**：SubAgent 和主 Agent 区别？ · 为什么 Conductor 不执行？ · 代码在哪个目录？
