...
export default [
  {
    ...
    analogy: '像导航软件：到路口看红绿灯（条件边），走错了可以掉头回来（回边），而不是一条单行道开到头。',
    plainTalk: `LangGraph 的循环不是靠外面套 while True，而是在图里画一条「回头路」——比如 tool 节点执行完，用 add_edge 连回 agent 节点，形成 ReAct 闭环。每转一圈，state 都会更新，还能被 checkpoint 存下来。条件边 should_continue 就像路口红绿灯：有 tool_calls 就去调工具，没有就 END 结束。这比裸 while 强在：循环边界看得见、每轮状态可持久化、任意轮可以 interrupt 挂起等人审批。`,
    detailAnswer: `**是什么**：LangGraph 的循环通过「回边」（back edge）实现——节点 A 执行完后，边可以指回之前的节点 B，形成环。经典 ReAct 模式是 agent_node → 条件边判断 → tool_node → 回边回到 agent_node，直到 LLM 不再发起 tool_calls 才走 END。
...
app = graph.compile()`,
    pitfalls: '只画回边不设退出条件会死循环；tool_node 回边方向写反会跳过 agent；忘记 recursion_limit 导致生产抛 500。',
    followupTips: `**「和 while True 区别？」**——强调三点：图上循环可 checkpoint/interrupt；每轮 state 显式演化；条件边可单测。while 适合脚本，LangGraph 适合要上线的 Agent。
...
面试要能手写 should_continue 伪代码，并说明为什么需要 step_count 或 recursion_limit 防止无限转。`,
  },
  {
    ...
    analogy: '像电梯的楼层限制和紧急停止钮——recursion_limit 是硬上限，step_count 是提前几层就提醒，语义查重是发现一直在按同一层按钮。',
    plainTalk: `Agent 死循环最常见的情况是：工具一直返回空，LLM 不死心反复重试，图就在 agent→tool→agent 之间空转。防法要设三道防线：框架级 recursion_limit 硬上限、state 里 step_count 软路由到 fallback、语义查重发现「同一工具同一参数调了 N 次」直接阻断。关键是不能让用户看到 GraphRecursionError 的 500，要在条件边里优雅降级，比如转人工或提示简化问题。`,
    detailAnswer: `**是什么**：死循环指图在环里无限执行，super-step 不断增长直到触发 recursion_limit 或耗尽 token/时间预算。ReAct Agent 里最常见诱因是工具返回空/错误，LLM 换汤不换药地重试。
...
**踩坑**：只依赖 recursion_limit 不设 fallback，用户看到 500；step_count 忘了在节点里递增导致软路由失效；语义查重太严误杀合理重试。`,
    codeExample: `def smart_router(state):
...
app.invoke(input, config={"recursion_limit": 20})`,
    pitfalls: '只靠 recursion_limit 用户体验差；step_count 未递增导致软路由失效；语义查重太严会误杀合理重试。',
    followupTips: `**「工具一直返回空怎么办？」**——分层处理：先 query_rewrite 换关键词重检；rewrite 仍失败走 fallback 坦诚告知；记录 failure_reason 进监控。
...
CSDN 高频题强调：单纯依赖 recursion_limit 用户体验差；在 smart_router 里引到 human_fallback，用户看到的是「这个问题较难，正在转人工」。`,
  },
  {
    ...
    analogy: '像电子游戏的「最大回合数」——到了就强制结束，但好的游戏会提前几回合给玩家提示，而不是突然黑屏。',
    plainTalk: `recursion_limit 是 LangGraph 框架给图执行设的「最大步数」上限，每经过一个 super-step 就计一次，超了抛 GraphRecursionError。默认值 1000 对 Agent 来说太高了，生产一定要显式调小。它和 state 里的 step_count 是互补关系：recursion_limit 是硬天花板，step_count 是业务软路由，可以在触顶前优雅走 fallback。`,
    detailAnswer: `**是什么**：recursion_limit 限制图执行的最大 super-step 数。一个 super-step 可能包含一个或多个并行节点的执行。超过限制，框架抛出 GraphRecursionError 终止执行。
...
app = graph.compile(checkpointer=saver, interrupt_before=["execute"])`,
    pitfalls: '默认 1000 太大，生产必须显式调小；只依赖硬限制不设 fallback 体验差；所有场景共用一个值不合理。',
    followupTips: `**「和 step_count 区别？」**——recursion_limit 是框架兜底抛异常；step_count 是业务可控的软路由。好的实践是 step_count 在 80% 阈值就走 fallback，recursion_limit 作为最后保险。
...
定值看业务：简单问答 5-10 轮，代码修复循环 15-20 轮。要结合 token 预算和 P99 延迟一起调。`,
  },
  {
    ...
    analogy: '普通边是单行道指示牌「前方直行」；条件边是分岔路口的交警，看一眼路况（state）再决定往左还是往右。',
    plainTalk: `普通边是「A 做完一定去 B」的固定跳转，适合流水线步骤。条件边是动态的：每次执行都跑一个路由函数，读当前 state 决定下一跳去哪。Agent 的核心决策——要不要调工具、检索质量够不够、要不要人工审批——全在条件边上。路由函数最好写成纯函数方便单测，复杂逻辑可以拆成独立 router 节点。`,
    detailAnswer: `**普通边（add_edge）**：确定性跳转，源节点执行完必然进入目标节点。适合固定流水线，如 retrieve → generate → format。
...
**踩坑**：在路由函数里写副作用（发请求、改数据库）；路由函数过于复杂难以测试；mapping 漏了 path_fn 可能返回的 key 导致运行时错误。`,
    codeExample: `def route_after_grade(state):
...
graph.add_edge("retrieve", "grade")  # 普通边：固定跳转`,
    pitfalls: '路由函数里写副作用难测试；mapping 漏 key 运行时报错；复杂 LLM 逻辑塞在 path_fn 里难以维护。',
    followupTips: `**「路由函数能调 LLM 吗？」**——可以但建议拆成 router 节点，条件边只做查 state 里的 route_key。强调可测试性和关注点分离。
...
多个出口就在 mapping 里多几个 key，比如 "retry"、"fallback"、"end"。`,
  },
  {
    ...
    analogy: '像游戏的「读档并选择分支」——Command(resume=...) 是带着新信息继续，Command(goto=...) 是强制跳转到另一个关卡。',
    plainTalk: `Command 是 LangGraph 1.0 里控制执行流的「遥控器」，专门用于 interrupt 之后恢复执行。人工审批通过后，不是简单 invoke(None)，而是用 Command(resume=审批结果) 把人的输入带回图里继续跑。也可以 Command(goto="某节点") 强制跳转，比如审批拒绝直接跳到道歉节点。面试提到 Command 说明你跟过 1.0 变化，不是只背 interrupt_before 老 API。`,
    detailAnswer: `**是什么**：Command 是 LangGraph 1.0 引入的执行控制原语，用于表达「从 interrupt 点如何继续」的意图。可以携带 resume 值（人工输入）、指定 goto 目标节点、或组合使用。
...
**踩坑**：resume 后 pending 边可能重跑节点，涉及副作用要幂等；混淆 Command 和 update_state 导致状态改了但执行流不对。`,
    codeExample: `from langgraph.types import Command
...
for event in app.stream(Command(resume={"approved": True}), config):
    handle(event)`,
    pitfalls: 'resume 后 pending 边可能重跑副作用节点；混淆 Command 和 update_state；忘记传同一个 thread_id config。',
    followupTips: `**「和 update_state 区别？」**——update_state 改快照；Command 改快照 + 表达执行意图（resume/goto）。审批改数据用 update_state，审批后继续/跳转用 Command。
...
面试提到 Command 说明跟过 1.0 变化，不是只背 interrupt_before 老 API。`,
  },
  {
    ...
    analogy: '像公司老板把任务拆给多个员工同时做（Send fan-out），最后秘书汇总报告（reducer/reduce 节点）。',
    plainTalk: `Send API 解决的是「运行时才知道要并行几份」的问题。一个节点 return [Send("worker", {"item": x}) for x in items]，框架会并行调度多个 worker 节点各自处理。和写死多条并行边不同，任务数量是动态的。结果合并靠 state 上的 reducer 或专门的 reduce 节点汇总。适合批量文档摘要、多源并行检索这类 Map 场景。`,
    detailAnswer: `**是什么**：Send 是 LangGraph 的动态 fan-out 原语。节点可以返回一个或多个 Send 对象，每个指定目标节点名称和传入的 state 片段，框架在同一 super-step 并行调度这些 worker。
...
**项目例子**：编程导航面经里的「多路调用」——检索、工具、规则引擎三路 Send 并行，汇总节点 merge 后交给 generate。`,
    codeExample: `from langgraph.types import Send
...
graph.add_conditional_edges("split", fan_out)`,
    pitfalls: '并行写同一字段没配 reducer 会丢数据；worker 失败没标 error 导致 reduce 卡住；Send 数量过大要限流。',
    followupTips: `**「和静态并行边区别？」**——静态分支数固定、每次都跑；Send 运行时决定数量和参数。举例「文档段数不固定」。
...
编程导航面经里「多路调用」可以结合 Send 讲——检索·工具·规则多路并行，汇总节点 merge。`,
  },
  {
    ...
    analogy: '像工厂流水线：切割车间把大板拆成零件（Map），各工位同时加工（worker），质检车间组装成品（Reduce）。',
    plainTalk: `Map-Reduce 在 LangGraph 里就是 Send + reducer 的组合拳。splitter 节点把大任务拆成 N 份，Send 给 worker 并行处理，reduce 节点把各 worker 结果合并——可以拼接、投票或再调 LLM 综合。部分 worker 失败时让它返回 error 标志，reduce 决定跳过或重试，别让一颗老鼠屎坏整锅粥。`,
    detailAnswer: `**Map 阶段**：splitter 节点将输入拆分为子任务列表，通过 Send API fan-out 到 worker 节点。每个 worker 独立处理一个子任务，写回 state 的对应字段（需 reducer 合并）。
...
**踩坑**：worker 抛异常导致整图失败；reduce 节点等所有 worker 但某个永远不回；合并时 token 超限要分段 reduce。`,
    codeExample: `def split_docs(state):
...
def reduce_node(state):
    ...
    pitfalls: 'worker 抛异常整图失败；reduce 等不到慢 worker；合并结果 token 超限要分层 reduce。',
    followupTips: `**「部分 worker 失败怎么办？」**——worker 返回 error 标志不抛异常；reduce 统计成功率决定继续/重试/fallback；可 Send 重试失败 chunk。
...
和 Hadoop Map-Reduce 思想一致，但粒度是 Agent 节点而非机器节点。`,
  },
  {
    ...
    analogy: '像导航「无法到达目的地，已为您规划备选路线」——不是突然崩溃，而是体面地告诉用户发生了什么、接下来怎么办。',
    plainTalk: `fallback 不是 catch-all 异常处理，而是图里一个正经的「降级出口」节点。当重试耗尽、检索质量不达标、step_count 超限等条件满足时，条件边路由到 fallback，返回用户能看懂的话——「建议简化问题」或「已转人工」——同时把 failure_reason 写进 state 供监控。和 retry 的边界是：可重试的先回环，确认没救了才 fallback。`,
    detailAnswer: `**是什么**：fallback 是图中的一个专用节点，负责在无法正常完成任务时返回降级响应，是条件边的一个出口而非全局异常捕获。
...
**踩坑**：fallback 消息太技术化吓用户；所有错误都走 fallback 导致 retry 形同虚设；没记录原因无法做归因分析。`,
    codeExample: `def route_on_error(state):
...
def fallback_node(state):
    ...
    pitfalls: 'fallback 消息太技术化；所有错误都走 fallback 导致 retry 无效；不记录 failure_reason 无法归因。',
    followupTips: `**「fallback 和 retry 边界？」**——可重试（超时/限流）先 retry；不可重试（参数非法）或次数耗尽才 fallback。给具体分类表。
...
和 retry 边界：可重试错误先回环，不可重试或超过 N 次才 fallback。别让 fallback 变成万能垃圾桶，要分类统计触发原因。`,
  },
  {
    ...
    freq: '3+ 篇面经',
    source: '牛客 · 某大厂/阿里国际',
    conclusion: '分三层：node 内 try-catch + 回边；tool 层超时重试 + 幂等；RAG 链层检索失败走 query rewrite 环。',
    followups: '副作用怎么幂等？ · 重试监控看什么？',
    keyPoints: ['可重试 vs 不可重试错误分类', '外部副作用配幂等键', '全局熔断：连续失败转人工'],
    analogy: '像快递三层投递：驿站重派（node）、快递员换路线（tool）、地址改写再寄（RAG rewrite），最终找不到才退回发件人（fallback）。',
    plainTalk: `牛客高频题，建议按三层答。Node 层用 try-catch 区分可重试/不可重试错误，可重试走回边。Tool 层设超时和 max_retries，有副作用的操作必须配幂等键。RAG 链层检索为空不走幻觉，而是走 query_rewrite 换关键词重检。全局加连续失败熔断，超阈值转人工，每层记录 attempt 次数进 trace。`,
    detailAnswer: `**Node 层**：每个节点外包 try-catch，将错误分类。可重试错误（网络超时、429 限流、临时 5xx）通过条件边路由到 retry 节点或回上一节点；不可重试错误（参数非法、401 鉴权失败）直接走 fallback。节点内维护 retry_count 字段。
...
**踩坑**：重试副作用操作导致重复扣款；不区分可重试/不可重试浪费配额；RAG 检索失败让 LLM 幻觉回答。`,
    codeExample: `def retrieve_node(state):
    try:
...
def route_after_retrieve(state):
    ...
    return "fallback"`,
    pitfalls: '重试副作用操作重复扣款/发邮件；不分类错误浪费配额；RAG 失败让 LLM 幻觉。',
    followupTips: `**「副作用怎么幂等？」**——工具层 idempotency_key = 业务主键；state 维护 executed_actions 列表；恢复前先查外部系统状态。
...
RAG 链层：检索为空或 grade 不通过，走 rewrite_query 节点回到 retrieve，而不是让 LLM 硬编。全局加连续失败计数，超阈值熔断转人工。每层记录 attempt 和最终状态进 trace。`,
  },
  {
    ...
    analogy: '像车陷泥坑一直轰油门——要检测「轮子空转」（重复动作），然后拖车（fallback）或换条路（rollback checkpoint）。',
    plainTalk: `stuck agent 就是图还在跑，但没有实质进展——同一工具同参数反复调、step_count 涨但关键字段不变、单 thread 耗时或 token 异常高。检测靠语义查重、进展监控、SLA 超时三条规则。处理上优先条件边引 fallback；严重的 interrupt 让人工看 state；LangGraph 支持 get_state_history 回滚到之前 checkpoint 再 resume。`,
    detailAnswer: `**表现**：同一工具同一参数反复调用；step_count 持续增长但 state 关键字段（如 retrieved_docs）不变；单 thread 执行时间或 token 消耗远超 P99 基线。
...
**踩坑**：只设超时没有 stuck 语义检测，误杀慢但正常的任务；rollback 不考虑已发生的副作用。`,
    codeExample: `def detect_stuck(state):
...
app.update_state(safe_cp.config, {"plan": revised_plan})`,
    pitfalls: '只靠超时会误杀慢任务；rollback 忽略已发生的副作用；不告警导致 stuck 长时间消耗资源。',
    followupTips: `**「怎么线上发现？」**——LangSmith trace + 单 thread 耗时告警 + repeat_count 指标。举例 dashboard 看哪些节点最容易 stuck。
...
线上用 LangSmith 或自研 trace 看节点级耗时，对 P99 异常的 thread 告警。`,
  },
  {
    ...
    analogy: '像医院分诊台——护士先问症状（语义分类），再分到内科/外科/急诊，而不是所有病人都排同一个队。',
    plainTalk: `语义路由就是在图入口用 LLM 或 embedding 分类用户意图，然后条件边根据分类结果跳到不同分支。实现上单独建 router 节点，输出 state 里的 route_key，条件边查表跳转。为省 token 可用小模型或 embedding+阈值。路由不稳时加澄清节点 interrupt 问用户，或低置信走安全默认路径。蚂蚁面经里的 LangGraph 路由设计是同类题。`,
    detailAnswer: `**是什么**：语义路由根据用户输入的语义（而非关键词规则）决定后续走哪条分支。可以用 LLM 分类、embedding 相似度匹配、或专用 fine-tuned 小模型。
...
graph.add_conditional_edges(
    ...
    pitfalls: '用大模型路由成本高；分类类别过多准确率差；没有低置信兜底导致误路由。',
    followupTips: `**「路由不准怎么办？」**——低置信走澄清 interrupt；默认安全路径；bad case 回流评测集迭代分类器。
