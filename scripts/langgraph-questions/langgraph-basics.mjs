...
export default [
  {
    ...
    oralAnswer: `LangGraph 是建立在 LangChain 核心之上的一款编排库，专门构建有状态、多参与者的循环计算图。底层可以看成状态机：全局 State 在各 Node 之间流转，Conditional Edge 决定下一步去哪。
...
我会强调它不是「更高级的 LangChain」，而是补上了 Chain 缺的循环、显式状态和生产级 checkpoint。简单 RAG 问答仍用 Chain 更合适；一旦流程有环、有审批、有跨请求恢复，LangGraph 的优势才明显。`,
  },
  {
    ...
    oralAnswer: `LangChain 核心是 Chain 和 LCEL，用管道把 retriever、prompt、LLM 串起来，适合标准化 RAG 和单次问答。它的控制流基本是静态的，一旦你要 loop、回退、等人审批，外面得套 while 或把逻辑塞进 prompt。
...
选型上：固定三四步用 Chain；有循环依赖、要持久化中间状态、要人机协同中断点，三个占一个就该上 LangGraph。工程上我常混用——编排用 LangGraph，底层组件用 langchain_core 的 message、retriever。`,
  },
  {
    ...
    oralAnswer: `我选 LangGraph 不是因为追新，而是业务确实需要图编排。比如有条件路由、失败重试、审批挂起——用 Chain 写会把分支和恢复散落在各处，难维护也难 debug。LangGraph 把 checkpoint 和 interrupt 做成一等公民，用在审批或补全流程里，省掉的胶水代码很实在。
...
过度设计不是道德问题，是可量化问题：每条条件边对应真实业务分支吗？checkpoint 里存的东西有生命周期策略吗？图内 state 和外置记忆划界了吗？答不上来可能确实用重了。面试时我会先讲业务因果链，再引出 LangGraph 作为承载工具，顺序不能反。`,
  },
  {
    ...
    oralAnswer: `被称为图状工作流，是因为它不把 Agent 当成黑盒循环，而是把执行流程建模成有向图。State 是共享数据结构，Node 是读 state、写 partial update 的函数，Edge 定义逻辑流向——普通边固定跳转，条件边根据 state 动态路由。
...
面试加分点是能说出 super-step 执行模型：每个 super-step 里所有就绪节点并行跑，跑完同步 state，再进入下一步。类似 Pregel，所以能处理并行分支又保证一致性。`,
  },
  {
    ...
    oralAnswer: `StateGraph 是 LangGraph 里定义图的核心类。你先用 TypedDict 或 Pydantic 定义 AgentState，告诉框架有哪些 channel、哪些字段用什么 reducer 合并。然后通过 add_node 注册计算单元，add_edge 和 add_conditional_edges 连边，set_entry_point 或 add_edge(START, ...) 定入口。
...
和 MessageGraph 比，StateGraph 更通用——State 可以是任意 TypedDict；MessageGraph 是 messages 列表的特例，底层仍基于 StateGraph，只是帮你预置了 add_messages reducer。`,
  },
  {
    ...
    oralAnswer: `Node 是图里的工作单元，本质就是 Python 函数，接收当前 state，返回要合并的更新字典。可以是 LLM 调用、工具执行、规则判断，任何逻辑都行。注意节点不应直接 mutate state，只返回 update。
...
START 和 END 是虚拟节点，标记入口和终止。条件路由不稳定是常见坑，我会加 fallback 边和超时计数，避免 LLM 路由飘到错误分支。`,
  },
  {
    ...
    oralAnswer: `compile 是把图定义变成可运行实例的步骤。它会做基本结构检查，比如有没有 unreachable 节点，然后在编译参数里绑定 checkpointer、interrupt 点、recursion_limit 等运行时配置。
...
图定义变更后，启用 checkpointer 的 thread 可能需要迁移策略——state schema 向后兼容、新增字段给默认值、灰度期间新旧图并行。`,
  },
  {
    ...
    oralAnswer: `END 是 LangGraph 的虚拟终止节点。条件边路由函数可以返回 END，表示任务完成、不再继续。显式终止很重要，因为 Agent 循环里「什么时候停」是核心问题——不能全靠 recursion_limit 硬砍。
...
我的做法是条件边同时看两个信号：LLM 是否不再请求工具（任务完成），以及 state 里的 step_count 或业务标志位。路由映射里一定要有到 END 的路径，否则图可能一直在环里转直到抛 GraphRecursionError，用户体验是 500 而不是优雅结束。`,
  },
  {
    ...
    oralAnswer: `MessageGraph 可以理解为 StateGraph 的简化版，state 主要就是 messages 列表，内置 add_messages 做消息追加和去重。适合简单聊天 Agent，代码更短。
...
面试时可以说：MessageGraph 是快速原型，StateGraph 是生产级定制。`,
  },
  {
    ...
    oralAnswer: `普通 Workflow 或 LCEL Chain 是线性的：检索、生成、格式化三步走完。中间某步失败，通常只能整链重试，没有「回到某步换策略」的能力，也没有跨请求的 state 持久化。
...
我同意高德面经里的观点：先用 Workflow 跑通功能，真正遇到循环/回溯痛点再迁 LangGraph。比如保存草稿这种固定三步，Workflow 够用；代码生成→运行→改错循环才需要图。`,
  },
  {
    ...
    oralAnswer: `这三个框架哲学不同。LangGraph 把 Agent 系统当状态机建，你先定义状态、节点、边、终止条件，可控性和可恢复性最好，适合长链路、要审计、要 HITL 的生产系统，代价是前期设计重。
...
我的决策树：怕失控、要 checkpoint、链路复杂 → LangGraph；今天就要 demo → CrewAI；研究多 Agent 对话 → AutoGen。长期跑的生产系统，LangGraph 优势会随复杂度上升而放大。`,
  },
  {
    ...
    oralAnswer: `Agent 是系统能力：能根据目标自主调用工具、多轮推理、处理反馈。LangGraph 不负责「让 LLM 变聪明」，它负责把 Agent 的执行流程搭成可观测、可恢复的状态机。
