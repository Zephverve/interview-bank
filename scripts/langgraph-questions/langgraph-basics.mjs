...
export default [
  {
    ...
    conclusion:
      'LangGraph 是 LangChain 生态里的图编排框架，把 Agent 工作流建模成有状态的状态机，原生支持循环、分支、持久化和人机协同，适合需要多步决策和纠错的复杂 Agent。',
    followups: '和 LangChain 具体差在哪？ · 简单任务会不会过度设计？ · 执行模型是什么？',
    keyPoints: [
      '本质是状态机引擎：State 共享、Node 转移、Edge 路由',
      '相比 DAG Chain，原生支持 Cycles（ReAct 闭环、重试、审批挂起）',
      '适合「思考→行动→观察→再思考」的多轮 Agent，不适合一次性问答',
    ],
    analogy: 'LangGraph 像给 Agent 配了一张带存档功能的流程地图，而不是一条只能往前走的单行道。',
    plainTalk:
...
app = graph.compile()`,
    followupTips: `- 追问「和 LangChain 差在哪」：强调 Chain 是线性 DAG，LangGraph 原生支持环 + checkpoint\n- 追问「是否过度设计」：用「有没有真实分支/循环/审批」三问自证\n- 追问「执行模型」：提 super-step 并行 + reducer 同步\n- 追问「简单任务」：承认固定三步用 Chain 更合适，不必炫技`,
    pitfalls: '把 LangGraph 当「更高级的 LangChain」而非「补循环和持久化的编排层」；简单问答也上图；忽略 compile 应全局单例复用。',
    oralAnswer: `LangGraph 是 LangChain 生态里的图编排库，把 Agent 工作流建成有状态的状态机：Node 是步骤，State 是共享数据，Edge 决定跳转。它适合 Agent 是因为真实任务常需「思考→调工具→观察→再思考」的 ReAct 闭环，Chain 的线性 DAG 写起来别扭。LangGraph 原生支持循环、条件分支、checkpoint 断点续跑和 interrupt 人工审批。简单 RAG 问答仍用 Chain；一旦出现环、审批或跨请求恢复，LangGraph 优势才明显。`,
  },
  {
    slug: '002-vs-langchain',
    ...
    conclusion:
      'LangChain 是线性 DAG 流水线，适合固定步骤 RAG；LangGraph 是图状态机，支持循环、条件分支、显式 State 和 checkpoint，适合复杂 Agent。',
    followups: '什么场景必须用 LangGraph？ · 和纯 while 循环比优势？ · 能否混用？',
    keyPoints: [
      'LangChain：无状态/外部管记忆，A→B→C 单向',
      'LangGraph：显式 State + reducer，可回边形成循环',
      'LangGraph 独有：interrupt、checkpointer、thread_id 会话隔离',
    ],
    analogy: 'LangChain 像工厂流水线，产品从 A 站到 B 站到 C 站一路向前；LangGraph 像带岔路和回头的导航系统，可以绕圈、改道、中途停车。',
    plainTalk:
...
graph.add_edge("rewrite", "retrieve")  # 回边`,
    followupTips: `- 「必须用 LangGraph」：循环、HITL、多 Agent 协作，占一个就该上\n- 「和 while 循环比」：LangGraph 有 reducer、checkpoint、可观测，while 只是裸循环\n- 「能否混用」：能，编排 Graph + 组件 Chain 是常见实践\n- 「缺点」：学习曲线陡、简单场景更重`,
    pitfalls: '选型只看热度不看业务；把 LangGraph 当成 LangChain 替代品而非编排层；忽略两者可混用。',
    oralAnswer: `LangChain 是 Chain/LCEL 线性流水线，适合固定步骤 RAG 和单次问答，控制流基本静态。LangGraph 把流程建成有向图，Node 是步骤，State 是显式 schema，支持回边循环、条件分支、checkpoint 和 interrupt。选型：固定三四步用 Chain；有循环、持久化或人机协同，占一个就该上 LangGraph。工程上常混用——编排 Graph，组件用 langchain_core。`,
  },
  {
    slug: '003-why-choose-langgraph',
    ...
    conclusion:
      '选 LangGraph 是因为业务需要分支+状态+持久化；缺点是学习曲线陡、前期设计成本高；过度设计的判断标准是图的边是否对应真实业务分支。',
    followups: '为什么不直接用 Cursor/现成 Agent？ · 重做还会选吗？ · checkpoint 生命周期有规划吗？',
    keyPoints: [
      '价值：把分支、状态、人机协同从业务代码抽成一层',
      '缺点：简单线性三步场景反而更重；版本升级要跟进',
      '过度设计：条件边是否映射真实分支？checkpoint 有无 TTL？',
    ],
    analogy: '选 LangGraph 像给复杂业务配专业项目管理软件，而不是用 Excel 硬管——有用，但简单任务用 Excel 更快。',
    plainTalk:
...
图退化成「调模型→调工具→再调模型」三步直线仍用 Graph；没规划 checkpoint 生命周期导致膨胀；面试只讲框架不讲业务因果链。`,
    codeExample: `# 编译时注入持久化和 interrupt
app = graph.compile(
    checkpointer=PostgresSaver(conn),
    interrupt_before=["human_review"],
)`,
    followupTips: `- 「为什么不直接用 Cursor/现成产品」：要可控、可审计、可嵌入业务系统\n- 「重做还会选吗」：按实际分支数和持久化需求诚实回答\n- 「checkpoint 生命周期」：主动提 TTL、里程碑裁剪、外置记忆\n- 「缺点」：学习成本、前期设计、简单场景不划算`,
    pitfalls: '先讲框架再讲业务（顺序反了）；图只有一条直线却用 Graph；checkpoint 无生命周期规划。',
    oralAnswer: `我选 LangGraph 是因为业务需要条件路由、失败重试、审批挂起——用 Chain 会把分支和恢复散落各处。LangGraph 把 checkpoint 和 interrupt 做成一等公民，省掉大量胶水代码。缺点：学习曲线陡，要先画状态演化；图退化成三步直线则不划算。过度设计看三条：条件边是否映射真实分支？checkpoint 有无 TTL？图内外记忆是否划界？`,
  },
  {
    slug: '004-graph-workflow-framework',
    ...
    source: 'CSDN 高频题',
    conclusion:
      '因为它用 Node+Edge+State 显式建模控制流，支持条件分支、并行、循环，比隐式 Chain 更可审计、可测试。',
    followups: '和普通 Workflow 引擎差在哪？ · 图灵完备意味着什么？',
    keyPoints: [
      '有向图：节点是计算单元，边是路由逻辑',
      '条件边让 LLM/规则动态决定下一步',
      '编译期检查孤立节点，运行期 super-step 同步状态',
    ],
    analogy: 'LangGraph 像地铁线路图——每站是一个节点，换乘箭头是边，乘客信息（State）随线路流转，还能绕圈回始发站。',
    plainTalk:
...
条件边路由不稳定（LLM 返回意外 key）导致跑飞；没画 END 出口；不理解 super-step 导致并发写 state 冲突。`,
    codeExample: `graph.add_conditional_edges(
    "router",
    lambda s: s["next_step"],  # 纯函数读 state
    {"rag": "rag", "tool": "tool", "end": END},
)`,
    followupTips: `- 「和普通 Workflow 差在哪」：原生环 + reducer + checkpoint\n- 「图灵完备」：条件边 + 回边可表达任意控制流，但应克制使用\n- 加分：提 super-step 并行同步模型`,
    pitfalls: '条件边路由函数不稳定无 fallback；图结构复杂却无文档；忽略 compile 结构检查报错。',
    oralAnswer: `LangGraph 被称为图状工作流，因为它用 Node+Edge+State 显式建模控制流，而非黑盒循环。State 共享，Node 返回 partial update，条件边动态路由。相比传统 Workflow：原生支持环（ReAct/重试）、reducer 自动合并状态、checkpointer 可暂停恢复。super-step 模型：每步并行跑就绪节点，跑完同步 state 再进下一步。`,
  },
  {
    slug: '005-stategraph',
    ...
    conclusion:
      'StateGraph 是 LangGraph 的图构建器，规定 State schema，注册 Node 和 Edge，compile 后变成可执行的 Runnable。',
    followups: 'compile 做什么检查？ · StateGraph 和 MessageGraph 区别？',
    keyPoints: [
    ...
    analogy: 'StateGraph 像建筑蓝图工具——先定户型（State schema），再摆房间（Node）和走廊（Edge），最后验收（compile）才能入住。',
    plainTalk:
...
每请求 compile 一次（应启动时全局单例）；State schema 和节点返回字段不一致；忘记 set_entry_point 或 START 边。`,
    codeExample: `from langgraph.graph import StateGraph, START
...
app = graph.compile(checkpointer=memory)`,
    followupTips: `- 「compile 检查什么」：孤立节点、入口/出口可达性\n- 「和 MessageGraph」：MessageGraph 是 messages-only 特例\n- 「compile 后能否改图」：不能，改定义需重新 compile`,
    pitfalls: '每 HTTP 请求 compile；State schema 与节点返回值不匹配；生产环境未注入 checkpointer。',
    oralAnswer: `StateGraph 是 LangGraph 的图构建器。用 TypedDict 定义 AgentState，add_node 注册节点，add_edge/add_conditional_edges 连边，compile 后变成可执行 Runnable。compile 做结构检查并注入 checkpointer、interrupt。编译后的 app 支持 invoke、stream、get_state。MessageGraph 是仅管理 messages 的特例，复杂 Agent 几乎都用 StateGraph。`,
  },
  {
    slug: '006-node-edge-conditional',
    ...
    source: 'CSDN + 官方文档',
    conclusion:
      'Node 是处理 state 的函数；Edge 是固定跳转；Conditional Edge 用路由函数根据 state 动态选择下一节点。',
    followups: 'Node 能放什么逻辑？ · 条件边不稳定怎么办？',
    keyPoints: [
      'Node：同步/异步 Python 函数，返回 partial state dict',
      '普通 Edge：A 执行完必定去 B',
      'Conditional Edge：path_function(state) 返回 key，映射到目标节点',
    ],
    analogy: 'Node 是工序，普通 Edge 是固定传送带，Conditional Edge 像分叉路口的红绿灯——看情况决定走哪边。',
    plainTalk:
...
节点内直接改 state 而非返回 update；条件边路由函数不稳定（LLM 输出飘）；路由 map 漏 key 导致 KeyError；节点里做不可重试的副作用（发邮件）无幂等。`,
    codeExample: `def route(state) -> str:
...
graph.add_edge("tools", "agent")`,
    followupTips: `- 「Node 能放什么」：LLM、工具、规则、任意 Python，但副作用要幂等\n- 「条件边不稳定」：加 fallback 边、step_count 上限、规则兜底\n- 「多个出口」：路由 map 里多个 key 指向 END 或不同分支`,
    pitfalls: '节点 mutate state；条件边无 fallback；路由函数依赖 LLM 自由文本而非结构化输出。',
    oralAnswer: `Node 是处理 state 的函数，返回 partial update，不直接 mutate state。普通 Edge 固定跳转。Conditional Edge 用路由函数读 state 返回 key，查表决定下一节点——Agent 动态决策的关键。ReAct 里 agent 节点后条件边看有没有 tool_calls，有则去 tools，tools 完普通边回 agent。START/END 是虚拟入口和终止。`,
  },
  {
    slug: '007-compile-graph',
    ...
    source: '官方 Graph API',
    conclusion:
      'compile 做结构校验、注入 checkpointer/interrupt 配置，产出可复用的 Runnable；生产环境应全局单例编译，不要每请求 compile。',
    followups: '编译后能改图吗？ · 图迁移怎么做？',
    keyPoints: [
      '检查图结构合法性（无孤立节点等）',
      '注入 checkpointer、interrupt_before/after',
      '编译一次全局复用，避免每请求开销',
    ],
    analogy: 'compile 像把建筑设计图盖章验收——验收通过才能正式使用，而且同一栋楼不用每次来人重新画图纸。',
    plainTalk:
...
config = {"configurable": {"thread_id": session_id}}`,
    followupTips: `- 「编译后能改图吗」：不能，改定义需重新 compile 新 app\n- 「图迁移」：state schema 向后兼容，新增字段给默认值，灰度新旧并行\n- 「性能」：强调全局单例，给出延迟对比`,
    pitfalls: '每请求 compile；图变更不考旧 thread 兼容；debug=True 留生产。',
    oralAnswer: `compile 把图定义变成可运行 Runnable：做结构检查（孤立节点等），注入 checkpointer、interrupt、recursion_limit。编译后支持 invoke/stream/batch。生产应在启动时 compile 一次全局复用，每请求 compile 是性能坑。图变更后 state schema 需向后兼容，灰度期间新旧图可并行。`,
  },
  {
    slug: '008-end-node',
    title: 'END 节点为什么重要？',
    ...
    conclusion: 'END 是显式终止符，条件边必须能路由到 END，否则图可能无限循环或无法判断完成。',
    followups: '怎么判断 Agent 该结束了？ · 多个出口怎么设计？',
    keyPoints: [
      'END 标记图执行终止，触发最终 state 返回',
      '条件边应包含到 END 的路径',
      '结合 recursion_limit 和业务完成标志双重保险',
    ],
    analogy: 'END 像马拉松终点线——没有明确终点，选手（Agent）可能一直绕圈跑直到被强制叫停。',
    plainTalk:
...
路由 map 无 END 路径；只靠 recursion_limit 不设业务终止条件；多个出口未区分「成功 END」和「失败 END」（应用不同 fallback 节点再 END）。`,
    codeExample: `from langgraph.graph import END
...
graph.add_conditional_edges("agent", should_continue, {"tools": "tools", "end": END})`,
    followupTips: `- 「怎么判断该结束」：无 tool_calls + 业务标志 + step 上限三重\n- 「多个出口」：成功 END vs fallback→END，state 里记录 exit_reason\n- 「和 recursion_limit」：END 是优雅终止，recursion_limit 是硬兜底`,
    pitfalls: '条件边无 END 路径；只靠 recursion_limit；不区分成功/失败退出。',
    oralAnswer: `END 是显式终止节点，条件边路由到 END 表示任务完成。Agent 循环必须能路由到 END，否则一直转到 GraphRecursionError。应同时看：LLM 不再请求工具、step_count 上限、业务完成标志。recursion_limit 是最后防线，不能替代显式 END 路径。`,
  },
  {
    slug: '009-messagegraph-vs-stategraph',
    ...
    source: 'GitHub 100 Questions',
    conclusion:
      'MessageGraph 是仅管理 messages 列表的特例，预置 add_messages；StateGraph 可定义任意 State 字段，更灵活。',
    followups: '聊天 Agent 用哪个？ · 多字段 state 何时必须 StateGraph？',
    keyPoints: [
      'MessageGraph：messages channel + add_messages reducer',
      'StateGraph：自定义 TypedDict，支持多业务字段',
      '复杂 Agent 几乎都用 StateGraph',
    ],
    analogy: 'MessageGraph 像只带一个「对话记录本」的简易版；StateGraph 像带多个文件夹（意图、检索结果、审批状态）的完整档案柜。',
    plainTalk:
...
复杂 Agent 仍用 MessageGraph 把业务字段塞进 message content；迁移时 checkpoint 旧 schema 不兼容。`,
    codeExample: `# MessageGraph：仅 messages
...
class AgentState(TypedDict):
    ...
    current_intent: str`,
    followupTips: `- 「聊天 Agent 用哪个」：纯聊天 MessageGraph 够；要带 RAG/工具状态用 StateGraph\n- 「何时必须 StateGraph」：除 messages 外有任何跨节点共享字段\n- 「迁移成本」：schema 变更 + 节点改造 + checkpoint 兼容`,
    pitfalls: '复杂 Agent 硬用 MessageGraph；业务数据塞进 message 文本；迁移不考虑 checkpoint 兼容。',
    oralAnswer: `MessageGraph 是 StateGraph 特例，只有 messages 且预置 add_messages，适合简单聊天 Agent。StateGraph 可定义任意 TypedDict 字段，每个配不同 reducer，适合有检索结果、路由标志、审批状态等的复杂 Agent。MessageGraph 快速原型，StateGraph 生产定制。`,
  },
  {
    slug: '010-vs-workflow',
    ...
    conclusion:
      'Workflow 线性 A→B→C，卡住只能整体重试；LangGraph 有环、有共享 State、有 checkpoint 和 HITL，适合多轮决策任务。',
    followups: '什么时候不必用 LangGraph？ · 迁移成本多大？',
    keyPoints: [
      'Workflow：无状态、线性、难中断',
      'LangGraph：可循环、可回溯、可人工介入',
      '固定三步任务 Workflow 20 行搞定，不必上图',
    ],
    analogy: 'Workflow 像一次性瀑布流——一步错整单重来；LangGraph 像可编辑的草稿——写到一半能保存、改道、请人过目。',
    plainTalk:
...
固定流程硬上图；该用 Graph 时用 while 裸循环；迁移时不做 state schema 设计直接翻译代码。`,
    codeExample: `# Workflow：线性，无环
chain = retriever | grader | generator

# LangGraph：带环 + checkpoint
graph.add_edge("run_code", "agent")  # 失败回 agent 改代码
app = graph.compile(checkpointer=saver)`,
    followupTips: `- 「不必用」：固定三步、无环、无审批、无跨请求恢复\n- 「迁移成本」：重构图结构 + 定义 State + 配 checkpointer，1-3 天视复杂度\n- 「能否用 Graph 做确定性 Workflow」：能，固定边即可，但简单场景过重`,
    pitfalls: '固定三步硬用 LangGraph；该用 Graph 时坚持用 Chain+while；迁移无 state 设计。',
    oralAnswer: `Workflow 线性 A→B→C，失败只能整链重试，难中断难持久化。LangGraph 有环、共享 State、checkpoint 和 HITL，适合多轮决策。必须用：循环、人工介入、多 Agent 协作。不必用：固定三步如检索→生成→格式化。先 Workflow 跑通，遇循环/回溯痛点再迁 LangGraph。`,
  },
  {
    slug: '011-vs-autogen-crewai',
    ...
    source: '知乎对比文 + 面经',
    conclusion:
      'LangGraph 强在可控、可审计、可恢复，适合严肃生产系统；CrewAI 快速原型；AutoGen 多 Agent 协作实验。',
    followups: '团队没有图编排经验怎么办？ · 长期维护哪个成本低？',
    keyPoints: [
      'LangGraph：显式状态机，学习曲线陡，链路最稳',
      'CrewAI：角色+任务快速成型，中等复杂度',
      'AutoGen：对话式协作表达强，行为更动态难控',
    ],
    analogy: 'LangGraph 像有完整施工图纸的工地；CrewAI 像快速组队的项目外包；AutoGen 像头脑风暴室——创意多但难控流程。',
    plainTalk:
...
# crew = Crew(agents=[...], tasks=[...])`,
    followupTips: `- 「团队无经验」：先 CrewAI 验证，再迁 LangGraph；或招/培一个图编排 owner\n- 「长期维护」：LangGraph 随复杂度上升优势放大，CrewAI 中等复杂度性价比最高\n- 「能否混用」：可以，LangGraph 编排 + 调用 CrewAI 子任务`,
    pitfalls: '生产用 AutoGen 无约束；复杂链路用 CrewAI 后期重构；选 LangGraph 团队无人维护。',
    oralAnswer: `LangGraph 显式状态机，可控可恢复，适合生产长链路和 HITL，学习曲线陡。CrewAI 角色+任务快速原型，中等复杂度。AutoGen 对话式协作，研究实验强，生产难控。决策：怕失控、要 checkpoint → LangGraph；快速 demo → CrewAI；多 Agent 对话研究 → AutoGen。`,
  },
  {
    slug: '012-langgraph-and-agent',
    ...
    source: '高德面经',
    conclusion:
      'Agent 是「能感知、决策、行动」的系统；LangGraph 是编排 Agent 循环、工具调用和分支的引擎，不是 Agent 本身。',
    followups: '能否用 LangGraph 实现确定性 Workflow？ · 黑盒 Agent 和白盒图区别？',
    keyPoints: [
      'Agent = LLM + 工具 + 记忆 + 规划循环',
      'LangGraph 提供循环、路由、状态持久化的运行时',
      '可用 LangGraph 实现 Workflow（固定边）或 Agent（条件边）',
    ],
    analogy: 'Agent 是会开车的人，LangGraph 是交通规则和道路系统——路不是司机，但没有路司机跑不远。',
    plainTalk:
...
以为 LangGraph 替代 LLM；黑盒 AgentExecutor 和 LangGraph 白盒图混为一谈；用 LangGraph 但所有决策仍塞 prompt 里（图退化为直线）。`,
    codeExample: `# Agent 循环：条件边 + 回边
graph.add_node("agent", call_llm_with_tools)
graph.add_node("tools", execute_tools)
graph.add_conditional_edges("agent", has_tool_calls, {"yes": "tools", "no": END})
graph.add_edge("tools", "agent")`,
    followupTips: `- 「确定性 Workflow」：能，全部用普通边、无 LLM 路由即可\n- 「黑盒 vs 白盒」：AgentExecutor 黑盒；LangGraph 每步可见可测\n- 「LangGraph 是 Agent 吗」：不是，是 Agent 的运行时/编排层`,
    pitfalls: 'LangGraph 当 Agent 本身；图退化成直线仍称 Agent；忽略 LangGraph 也可做确定性 Workflow。',
    oralAnswer: `Agent 是「感知、决策、行动」的系统能力；LangGraph 是编排 Agent 循环、工具调用和分支的引擎，不是 Agent 本身。LangGraph 解决循环怎么写、状态怎么传、失败从哪重试、何时等人审批。固定边是 Workflow，条件边+回边是 Agent。LangGraph 是白盒，每个节点可观测可单测。`,
  },
]
