...
面试画两张拓扑图比背定义强。`,
    plainTalk: `**一句话**：任务步骤能预先列清单 → Plan-and-Execute；得边做边想、工具调用不确定 → ReAct；实际常混合用。

**打个比方**：Plan-and-Execute 像按菜谱做菜（先列步骤再执行）；ReAct 像侦探破案（推理→行动→观察→再推理，下一步取决于上一步结果）。`,
    detailAnswer: `Agent 架构模式选型题，考察**能否根据任务结构选模式**，而不是背定义。
...
**面试技巧**：画两张拓扑图——Plan 是 DAG+replanner 回边；ReAct 是 agent↔tool 环——比背定义强十倍。`,
    codeExample: `# Plan-and-Execute 骨架
...
def executor_node(state):
    ...
    followupTips: `- **「能画两种拓扑吗？」** → Plan：planner → executor → (条件边) replanner/下一步/END；ReAct：agent ↔ tool 环 + should_continue 条件边；混合：plan 出 milestones，每个 milestone 是 ReAct 子图。`,
  },
  {
...
schema 和 @tool 装饰器定义同源，避免两套。高危 tool 还可加 policy 节点检查权限。`,
    plainTalk: `**一句话**：LLM 生成的 tool 参数不能 blindly 执行——加 validate 节点用 Pydantic 校验，不过就回 agent 让它重生成，过了才调真实 tool。

**打个比方**：像银行转账前的二次确认——不是用户说转多少就转多少，先校验金额格式、账户合法性，不对就退回让用户重新输入。`,
    detailAnswer: `工具参数校验是**生产安全的基础防线**——LLM 生成的 tool_calls 参数可能是错的、越界的、甚至恶意的。
...
**高危 tool 额外加 policy 节点**：检查用户权限（这个 user 能调 delete 吗？）、参数范围（删除数量 < 100？）、操作频率（5 分钟内不超过 3 次？）。`,
    codeExample: `class SearchArgs(BaseModel):
...
def route_after_validate(state):
    ...
    followupTips: `- **「和 JSON schema 关系？」** → Pydantic model 自动生成 JSON schema；@tool 装饰器的 args_schema 就是 JSON schema；validate 节点和 tool 定义同源，LLM 看到的 schema 和校验用的 schema 一致。`,
  },
  {
...
并行后必须 reducer 合并写同一 channel 的结果。IO 密集节点用 async 定义，图用 ainvoke。注意并行不是无脑越多越好，LLM 并发受 rate limit 约束。`,
    plainTalk: `**一句话**：LangGraph 并行有两种——静态扇出（同一前驱连多个后继自动并行）和动态 fan-out（Send API 按运行时数据分发）；结果靠 reducer 合并。

**打个比方**：静态并行像同时派三个侦察兵去不同方向；动态并行像根据敌人数量决定派几个兵，数量运行时才知道。`,
    detailAnswer: `并行执行是 LangGraph **性能优化的关键手段**，考察对 super-step 模型的理解。
...
class State(TypedDict):
    ...
    followupTips: `- **「异步 ainvoke 注意什么？」** → 节点定义 async def，图用 ainvoke/astream；别在 async 节点里调阻塞 IO（用 httpx async 或 asyncio.to_thread）；FastAPI 路由里 await graph.ainvoke 不堵 worker。`,
  },
  {
...
FastAPI 路由里 await graph.ainvoke 不堵 worker 线程。同步节点和异步节点可混用，框架会调度。`,
    plainTalk: `**一句话**：节点可以是 async def，图用 ainvoke 调用；但别在 async 节点里写阻塞代码（如 requests.get），会堵死 event loop。

**打个比方**：async 节点像异步电话——可以同时打多个电话等回复；但在异步电话里用对讲机（阻塞 IO）会卡住所有线路。`,
    detailAnswer: `异步节点是 LangGraph **高并发场景的基础**，考察对 Python async 的理解。
...
**和线程池关系**：\`asyncio.to_thread\` 把阻塞调用放到线程池，不阻塞 event loop；适合必须用的同步库（如某些 DB driver）。`,
    codeExample: `# 正确：async 节点 + async HTTP
...
async def chat(req):
    ...
    followupTips: `- **「和线程池关系？」** → asyncio.to_thread 把阻塞调用（如同步 DB driver）放到线程池执行，不阻塞 event loop；适合不得不用同步库的场景；但首选原生 async 库（httpx、aiohttp、asyncpg）。`,
  },
  {
...
多租户用 fair queue 或 tenant 级并发上限，避免一租户占满 worker。`,
    plainTalk: `**一句话**：限流分三层——API 网关限用户 QPS、图入口检查 quota、LLM 节点内 token bucket；429 错误走退避重试而不是硬怼。

**打个比方**：像高速公路收费站——入口限流（网关）、路段限速（节点内）、堵车了绕道（429 fallback），不能一辆车堵死整条路。`,
    detailAnswer: `限流是**生产级 Agent 的必答题**，考察多层防御思维。
...
**监控**：限流触发次数、429 比例、backoff 重试成功率——这些指标进 dashboard。`,
    codeExample: `async def llm_node(state, config):
...
def route_after_llm(state):
    if "retry_after" in state:
        return "backoff"
    return "next_node"`,
    followupTips: `- **「多租户公平调度？」** → fair queue 保证每 tenant 最小配额；tenant 级并发上限；监控每 tenant 的 QPS/token 消耗，异常 tenant 自动降级。`,
  },
  {
...
适合 A/B 测试 prompt、按租户切模型，不进 checkpoint 避免污染历史。和 state 区别：state 是业务数据跨步持久化，config 是本次运行参数。`,
    plainTalk: `**一句话**：configurable 是「本次运行的配置」——模型名、温度、租户 ID——通过 invoke 传入，节点从 config 参数读取，不进 checkpoint。

**打个比方**：state 是病人的病历（要存档），configurable 是今天用哪个科室的医生（临时指定，不需要写进病历）。`,
    detailAnswer: `configurable 是 LangGraph **运行时配置的标准机制**，考察对 state vs config 划界的理解。
...
**最佳实践**：configurable 放「运行环境参数」，state 放「业务数据」；不要把 model_name 写进 state，否则 checkpoint 会污染历史。`,
    codeExample: `def llm_node(state, config):
...
graph.invoke(state, config={"configurable": {"model": "gpt-4o", "tenant": "acme"}})`,
    followupTips: `- **「和 state 区别？」** → state 是业务数据、跨 step 持久化、进 checkpoint；configurable 是运行参数、单次 invoke、不进 checkpoint；model_name/tenant_id 放 configurable，query/docs 放 state。`,
  },
  {
...
副作用节点用 DB 事务，成功写 executed 标志进 state 再 commit。checkpoint 和业务库是两套存储，恢复时先查业务幂等再 resume 图。`,
    plainTalk: `**一句话**：state 里存 ID 不存大对象，节点内按需查库；图状态管编排进度，业务库管业务数据，用业务键关联。

**打个比方**：state 像工作台的便签纸（只写订单号），详细信息去档案柜（数据库）查；便签纸轻方便携，档案柜存完整记录。`,
    detailAnswer: `外部数据库集成考察**图状态与业务数据的划界**，是系统设计高频题。
...
async def payment_node(state):
    ...
    followupTips: `- **「双写一致性？」** → checkpoint 和业务库是两套存储，不追求强一致；用业务键关联 + 幂等标志（executed=True）；resume 前先查业务库确认副作用是否已发生，已发生则跳过。`,
  },
  {
...
裁剪后 checkpoint 存的是裁剪后 state，有意控制体积。和压缩节点可串联：先 summarize 再 trim。`,
    plainTalk: `**一句话**：在 agent 节点前加 trim 节点，按 token 预算保留 system + 最近 k 轮，旧的 summarize 或删除，控制送进 LLM 的上下文大小。

**打个比方**：像整理桌面——只留当前任务需要的文件（最近 k 轮），旧文件归档到文件夹（summary 字段）或扔掉（RemoveMessage）。`,
    detailAnswer: `Message Trimming 是**控制 token 成本和上下文窗口的关键手段**，字节/阿里面经高频。
...
**和压缩节点串联**：先 compress（summarize 旧消息）→ 再 trim（按 token 硬截断），双保险。`,
    codeExample: `def trim_node(state):
...
builder.add_edge("trim", "agent")  # trim 在 agent 前`,
    followupTips: `- **「和 checkpoint 冲突吗？」** → 不冲突，裁剪后 checkpoint 存裁剪后的 state 是有意设计；完整历史外置存储，trim 只影响送进 LLM 的上下文；resume 时从裁剪后的 state 继续，不会恢复已删消息。`,
  },
  {
...
LangSmith experiment 记录每版图变化。非确定性用 LLM-as-judge 或结构匹配。和百度面经评测闭环结合答更完整。`,
    plainTalk: `**一句话**：LangGraph 评测分两级——端到端看最终答案+轨迹，节点级看每个 node 的输入输出；LangSmith 跑批量回归。

**打个比方**：端到端像考最终成绩，节点级像考每科分数——总分高但数学不及格，说明 generate 节点有问题。`,
    detailAnswer: `LangGraph Agent 评测考察**是否理解图编排的评测优势**——能评轨迹，不只是评最终答案。
...
def test_grade_node():
    ...
    followupTips: `- **「非确定性怎么评？」** → LLM-as-judge 评质量；结构匹配（JSON schema/关键词/citation 数）；多次采样取成功率；节点级评测用 mock state 测确定性部分（如 intent 分类）。`,
  },
  {
...
检索节点加 metadata filter tenant_id。防止 tenant A 的 thread 加载 tenant B 数据靠网关层校验 config。`,
    plainTalk: `**一句话**：多租户靠三层隔离——thread_id 含 tenant_id、checkpointer 分区存储、configurable 注入租户配置和工具权限。

**打个比方**：像 SaaS 公寓——每个租户有自己的房间号（thread_id）、自己的储物柜（checkpoint 分区）、自己的门禁权限（tool 白名单）。`,
    detailAnswer: `多租户设计是**B 端 Agent 产品的必答题**，考察数据隔离和配置隔离。
...
def retrieve_node(state, config):
    ...
    return {"docs": docs}`,
    followupTips: `- **「数据隔离怎么做？」** → 向量库 metadata filter tenant_id；业务 DB 所有查询带 tenant_id；checkpointer 分区存储；网关层校验 tenant_id 与用户身份匹配；工具权限按 tenant 白名单。`,
  },
  {
...
提示注入靠 system 边界+检索内容隔离+不要 tool 结果直接当 system。和百度 Guardrails 输入输出可一起讲。`,
    plainTalk: `**一句话**：Agent 安全分五层——输入过滤、工具白名单、HITL 审批、输出过滤、secret 不进 state。

**打个比方**：像机场安检——入口安检（输入过滤）、登机口验票（工具权限）、危险品需额外审批（HITL）、出口检查（输出过滤）、机密文件不进普通行李（secret 隔离）。`,
    detailAnswer: `Agent 安全是**生产上线的硬性要求**，考察多层防御思维。
...
**提示注入防御**：system prompt 边界清晰 + 检索内容用 user 角色注入（不当 system）+ tool 结果不直接拼进 system prompt。`,
    codeExample: `def sanitize_node(state):
...
graph = builder.compile(
    ...
    followupTips: `- **「提示注入怎么防？」** → system 边界清晰 + 检索内容用 user 角色注入 + tool 结果不当 system + 输入 Guardrails 节点检测注入模式 + 输出过滤敏感信息。`,
  },
  {
...
和 cron 结合：定时器触发 batch 子图处理积压。图编译一次，每事件一次 invoke，state 隔离。`,
    plainTalk: `**一句话**：事件驱动 = 外部事件（Kafka/Webhook）触发 invoke 图，每事件一个新 thread，event_id 做幂等键防重复。

**打个比方**：像外卖订单系统——每来一个订单（事件）触发一次处理流程（invoke 图），订单号（event_id）防重复下单。`,
    detailAnswer: `事件驱动 Agent 考察**图编排与非交互式场景的结合**。
...
**关键原则**：图 compile 一次（应用启动时），每事件一次 invoke，不 per-event compile。`,
    codeExample: `# Worker 消费 Kafka 事件
...
# 重复消费 → checkpoint 已存在 → 跳过`,
    followupTips: `- **「和 cron 结合？」** → 定时器触发 batch 子图：query 积压事件 → Send API fan-out 并行处理；适合非实时场景；cron 负责「什么时候跑」，图负责「怎么跑」。`,
  },
  {
...
单节点调试 mock state 直接调节点函数，不必跑全图。面试带一张自己项目的 Mermaid 图很加分。`,
    plainTalk: `**一句话**：可视化用 draw_mermaid_png 导出图结构；调试用 stream_mode="values" 看每步 state 变化；LangSmith 看完整轨迹。

**打个比方**：可视化像看地图（图结构），调试像看行车记录仪（每步 state 变化），LangSmith 像黑匣子（完整轨迹回放）。`,
    detailAnswer: `可视化和调试是**开发效率的关键**，也是面试加分项。
...
result = grade_node({"docs": mock_docs, "query": "test"})`,
    followupTips: `- **「给面试官画过图吗？」** → 准备一张自己项目的 Mermaid 图（PNG 或手绘）；面试时 30 秒画完核心拓扑（3-5 个节点 + 条件边）；重点展示环（ReAct）或回边（grade→rewrite）。`,
  },
  {
...
fallback 是终极出口；degradation 是沿途有备用方案。state 记 degradation_level 供监控统计各级占比。`,
    plainTalk: `**一句话**：优雅降级是「主路径不行走备用路径，备用还不行走兜底」——用户总能拿到结构化响应，不是 500 错误。

**打个比方**：像GPS导航——首选高速（GPT-4o+向量检索），高速堵了走国道（mini+缓存），国道也堵了给你文字指引（模板回复），总比说「找不到路」强。`,
    detailAnswer: `优雅降级考察**生产环境的容错设计**——用户感知到的是「慢/简/缺」，不是「挂了」。
...
**监控**：\`state["degradation_level"]\` 记录当前级别，统计各级占比——如果 Level 2/3 占比 > 10%，说明主路径有问题需要修。`,
    codeExample: `def route_after_primary(state):
    if state.get("llm_timeout"):
        return "degraded_llm"
    if state.get("retrieval_empty"):
...
state["degradation_level"] = 2  # keyword_fallback`,
    followupTips: `- **「和 fallback 区别？」** → degradation 是沿途多级备用（质量逐步降但仍有价值）；fallback 是终极出口（模板/人工）；degradation 是「尽量服务」，fallback 是「不服务了但给个交代」。`,
  },
  {
...
长期记忆外置 mem0/Postgres，retrieve 节点按意图回填。字节面经长期记忆召回可接这个答法。`,
    plainTalk: `**一句话**：不是无脑删旧消息，而是根据当前意图选择性保留相关历史——聊订单保留订单轮次，闲聊压缩掉。

**打个比方**：像整理笔记本——不是把旧页全撕了，而是当前写数学就只留数学相关页，语文笔记归档到文件夹。`,
    detailAnswer: `上下文感知记忆压缩是**比简单 trim 更智能的策略**，字节面经长期记忆考点。
...
- 图内管「当前任务上下文」，图外管「跨会话记忆」`,
    codeExample: `def compress_node(state):
    ...
    followupTips: `- **「和字节动态长期记忆？」** → 同一思想：compress 节点按 intent 选择性压缩 + retrieve 节点按 intent 回填 + 外置 mem0/Postgres 存长期记忆；字节考动态召回，LangGraph 考节点实现。`,
  },
  {
...
告诉用户「处理超时，可继续等待或简化问题」比 504 裸错误好。`,
    plainTalk: `**一句话**：超时处理分两层——节点级用 asyncio.wait_for 限时，超时走 fallback；图级 API 设总 SLA，超时 cancel 任务。

**打个比方**：像外卖配送——每道工序有截止时间（节点级），整个订单有总时限（图级 SLA），超时不取消订单而是告知用户「还在做，可继续等或换简餐」。`,
    detailAnswer: `超时处理是**用户体验的关键**，考察节点级和图级两层超时设计。
...
**监控**：超时率按节点统计，P99 延迟超 SLA 比例进 alert。`,
    codeExample: `async def llm_node(state):
    try:
...
def route_after_llm(state):
    ...
    followupTips: `- **「cancel 后 checkpoint 状态？」** → 被取消的 step 不写入 checkpoint；checkpoint 保留 cancel 前最后成功的 state；resume 从那个 state 继续，被取消的 step 重跑；不会丢数据也不会重复执行已完成的 step。`,
  },
  {
...
Shadow mode：同输入跑 v2 不返回用户，对比 trace 和答案。回滚是流量切回 v1+停开 v2 thread。schema 版本跟 graph 版本一起走。`,
    plainTalk: `**一句话**：图版本管理 = 多实例共存 + 路由层选版本 + 新 thread 用新版、旧 thread 用旧版 finish。

**打个比方**：像手机系统升级——新买家用新系统（新 thread → v2），老用户可以选择不升级直到当前任务完成（旧 thread → v1 finish）。`,
    detailAnswer: `图版本管理是**生产迭代的必答题**，考察零停机升级思维。
...
async def chat(req):
    ...
    followupTips: `- **「schema 版本一起管吗？」** → 是，graph_version 和 schema_version 绑定发布；新增字段给默认值（向后兼容）；不兼容变更需要新 graph_version + 迁移脚本；旧 thread 的 checkpoint schema 不能 break。`,
  },
  {
...
字节面经成本题：估 thousand-line code token 数×单价，说优化方向是压缩、缓存、小模型路由。成本感知是 router 节点的业务规则。`,
    plainTalk: `**一句话**：成本感知路由 = 简单问题用小模型，复杂问题用大模型，超预算强制降级——在 router 节点做决策。

**打个比方**：像打车——短途走路/骑车（mini），长途打专车（4o），月预算花完了就只能走路（模板回复）。`,
    detailAnswer: `成本感知路由考察**商业意识和工程优化能力**，字节面经百万 token 成本是经典题。
...
**监控**：每 tenant 日 token 消耗、模型分布、成本/请求比。`,
    codeExample: `def router_node(state):
...
def route_by_cost(state):
    ...
    return "llm_4o" if state["complexity"] == "complex" else "llm_mini"`,
    followupTips: `- **「字节问百万 token 成本怎么答？」** → 估算 token 数 × 单价 × 调用量；优化三板斧：压缩（减 input）、缓存（命中重复 query）、小模型路由（80% 简单 query 走 mini）；给出具体数字显得有商业意识。`,
  },
  {
...
ModelEngine 社区说要点全中：reducer、START/END、返回 dict 合并。面试能脱稿写加分。`,
    plainTalk: `**一句话**：最小 Agent 就 10 行——MessagesState + llm_node + START→LLM→END；加 tool 变 ReAct 环，加 MemorySaver 变多轮。

**打个比方**：最小 Agent 像「Hello World」——证明你理解核心概念；加 tool 是「Hello World with input」，加 memory 是「Hello World with persistence」。`,
    detailAnswer: `手写最小 Agent 是**一面 P0 白板题**，必须能脱稿写。
...
**ModelEngine 社区要点**：reducer（add_messages）、START/END、返回 dict 自动合并——全中。面试能脱稿写加分，加讲每步含义更加分。`,
    codeExample: `from langgraph.graph import StateGraph, START, END, MessagesState
...
builder.add_edge("llm", END)
graph = builder.compile()`,
    followupTips: `- **「加 tool 怎么改？」** → 加 ToolNode + should_continue 条件边（有 tool_calls → tools，没有 → END）+ tools→llm 回边，变成 ReAct 环。
...
代价是设计成本——你得自己画流程，不能全扔给 LLM 自主。生产环境要审计、合规、排障，白盒是必须的，这就是 LangGraph 取代黑盒 Agent 的原因。`,
    plainTalk: `**一句话**：早期 Agent 是黑盒（给工具和目标，内部怎么跑看不清）；LangGraph 是白盒（每个节点、每条边都显式），可测可审计。

**打个比方**：黑盒像自动售货机（投币出饮料，不知道内部机制）；白盒像开放式厨房（每道工序看得见），出问题能定位到哪一步。`,
    detailAnswer: `黑盒 vs 白盒是**理解 LangGraph 价值的根本问题**。
...
**为什么生产选白盒**：审计（金融/医疗必须知道 AI 做了什么）、合规（每步可追溯）、排障（bad case 定位到具体 node）、评测（节点级回归）。这就是 LangGraph 取代黑盒 Agent 的根本原因。`,
    followupTips: `- **「白盒代价是什么？」** → 设计成本（先画流程再写代码）、灵活性降低（LLM 不能随意跳步）、前期投入大（State/reducer/节点契约）；但生产环境的审计/合规/排障需求使白盒成为必选项。`,
  },
  {
...
Trade-off：CrewAI 快但控不住；LangGraph 稳但重；LlamaIndex 检索强但不包全流程。实际项目常 LlamaIndex 检索 + LangGraph 编排。面试说 trade-off 比背名字强。`,
    plainTalk: `**一句话**：选型看核心需求——数据索引用 LlamaIndex，简单链用 LangChain，快速原型用 CrewAI，复杂可控流程用 LangGraph；可以组合使用。

**打个比方**：像选交通工具——运货用卡车（LlamaIndex），短途用自行车（LangChain），团建包车用巴士（CrewAI），复杂物流调度用调度系统（LangGraph）。`,
    detailAnswer: `框架选型决策树是**二面 P1 题**，考察全局视野而非只会一个框架。
...
**面试技巧**：说 trade-off 比背名字强——「CrewAI 适合快速验证，但如果要 HITL 和 checkpoint，生产一定上 LangGraph」。`,
    followupTips: `- **「能说出 trade-off 吗？」** → 每个框架一句话 trade-off：LlamaIndex 检索强但不包编排；LangChain 生态大但复杂流程难表达；CrewAI 快但控不住；LangGraph 稳但重；实际项目常组合使用。`,
  },
  {
...
和 Airflow 比：LangGraph 更轻、和 LLM 节点混排自然，适合 AI 流水线；重 ETL 还是 Airflow。很多团队用 LangGraph 管「含 LLM 步骤」的工作流，纯数据用传统调度。`,
    plainTalk: `**一句话**：LangGraph 可以不当 Agent 用——固定边 + 规则条件边（不调 LLM）就是确定性 Workflow，仍有 checkpoint 和 HITL 能力。

**打个比方**：LangGraph 像瑞士军刀——能当 Agent 用（LLM 决策），也能当 Workflow 用（固定流程），还能混合用（前面固定后面 Agent）。`,
    detailAnswer: `这道题考察**对 LangGraph 定位的理解**——它不只是 Agent 框架，也是 Workflow 引擎。
...
**实际用法**：很多团队用 LangGraph 管「含 LLM 步骤」的工作流（如内容审核：上传→LLM 审核→人工审批→发布），纯数据 ETL 仍用 Airflow。`,
    codeExample: `# 确定性 Workflow：条件边不调 LLM
def route_by_status(state):
...
builder.add_edge("publish", END)`,
    followupTips: `- **「和 Airflow 区别？」** → LangGraph 轻量、原生 LLM 集成、适合含 AI 步骤的流水线；Airflow 是通用 ETL 调度、重量、适合大数据批处理；实际常组合：Airflow 调度触发 LangGraph 图。`,
  },
  {
...
选型看 checkpoint 频率、保留多久、要不要 SQL 查询 thread 状态。v4 checkpointer keep_latest TTL 自动裁剪旧快照，生产要配。`,
    plainTalk: `**一句话**：checkpointer 可插拔——开发用 Memory、单机用 SQLite、生产用 Postgres、要极速用 Redis；按 checkpoint 频率和查询需求选。

**打个比方**：像选存储方案——开发用内存（MemorySaver），小项目用本地硬盘（SQLite），生产用云数据库（Postgres），缓存用 Redis。`,
    detailAnswer: `State 持久化策略考察**生产选型的工程判断**。
...
graph = builder.compile(checkpointer=MemorySaver())`,
    followupTips: `- **「自定义 Redis checkpointer 要点？」** → 序列化大小（state 可能很大，考虑 gzip 压缩）；TTL 每个 checkpoint 设 expire；命名空间 thread_id 作 key prefix；注意 Redis 单 value 大小限制（512MB）。`,
  },
  {
...
这道题把分散工程点串起来，二面收尾常考。`,
    plainTalk: `**一句话**：生产上线 12 项 checklist——编译单例、PG checkpoint、幂等、HITL、限流、监控、评测、降级、版本、secrets、文档化 state 约定。

**打个比方**：像飞机起飞检查清单——12 项全绿才放行；漏一项可能不是立即坠机，但会在某个恶劣条件下暴露。`,
    detailAnswer: `生产上线 Checklist 是**二面收尾题**，考察能否把分散工程点串成体系。
...
async def shadow_test(prod_input):
    ...
    followupTips: `- **「上线前最后一项检查什么？」** → shadow traffic：用生产真实输入跑新版本，对比 trace/节点失败率/token 消耗/答案质量，确认无退化才切流量；同时确认 state reducer 约定已文档化，oncall 能看懂。`,
  },
]
