...
什么时候用现成的：内部提效、非核心路径、合规要求不高时，Composer/企业 Agent 产品更快。核心业务流程、敏感数据、要强审计再上 LangGraph 自研。`,
    plainTalk: `**一句话**：不是非要证明自己比 Cursor 强，而是核心链路要「数据、权限、评测、发版」四条线都在自己手里。

**打个比方**：Composer 像租精装公寓——拎包入住快，但装修不能动、监控看不到管道；自研 LangGraph 像自建房——前期慢，但水电走向、门禁系统、消防验收全在自己图纸里。`,
    detailAnswer: `这道题表面在问「为什么不直接用现成产品」，实际考的是**技术主权边界**和**改造方案的可落地性**。
...
最后补一句选型原则：**非核心、低合规、要快** → 用 Composer/企业 Agent；**核心流程、敏感数据、强审计** → LangGraph 自研。面试官想听的是你知道 trade-off，不是站队。`,
    codeExample: `# adapter 四层示意：不改 fork，先包一层
class InternalAgentAdapter:
    ...
    async def invoke(self, user_id, query):
        ...
        return result`,
    followupTips: `- **「fork 现成方案有什么风险？」** → 上游 merge 成本、安全补丁滞后、团队只有维护权没有架构权；正确姿势是先 adapter 再接，验证 ROI 后再考虑深度 fork。
...
字节面经 skill 分层可接：静态 skill 库 + 动态匹配 + 沉淀机制，LangGraph 里 skill 选择可以是路由节点或 tool 子集，和 Skill 化是同一思想。`,
    plainTalk: `**一句话**：Skill 化不是把 prompt 存成 markdown 文件，而是把「一类任务的完整打法」变成可版本、可组合、可回归测试的工程资产。

**打个比方**：没有 Skill 化时，每个需求都是厨师临场发挥；Skill 化后，高频菜有标准菜谱（流程）、固定厨具（工具子集）、品控表（回归用例），新厨师照做也能出稳定品质。`,
    detailAnswer: `Skill 化的核心是**把 tacit knowledge（隐性经验）变成 explicit asset（显式资产）**，在 LangGraph 语境下具体落地为三层：
...
和 MCP 的关系：MCP 是**工具接入协议**（怎么连外部服务），Skill 是**任务编排资产**（怎么组合工具完成一类任务），两者正交——MCP 提供 tool，Skill 定义图路径。字节 skill 分层可接：静态库 + 动态匹配 + 沉淀机制，LangGraph 里用路由节点选 skill 子集。`,
    codeExample: `# Skill 化：路由节点选 tool 子集 + 子图
SKILL_REGISTRY = {
...
def route_skill(state):
    ...
    followupTips: `- **「和 MCP 关系？」** → MCP 管工具怎么连（协议层），Skill 管任务怎么打（编排层）；一个 MCP server 可以被多个 skill 复用。
...
接 RAG 检索时另一子图 retrieve→grade，入库和问答解耦。讲的时候边画 state 字段边讲条件边，比只背向量库 API 强。`,
    plainTalk: `**一句话**：把「数据落向量库」建模成一条 ETL 子图，每个步骤是一个 node，失败从 checkpoint 断点续跑，不重复已成功批次。

**打个比方**：像工厂流水线——原料入口分三条线（PDF/网页/结构化），每条线经过切分→向量化→装箱入库，任何一站停电（失败）从上一站成品接着干，不用从头再来。`,
    detailAnswer: `这是二面系统设计题，考察**能否把 ETL 流程用图编排讲清楚**，而不是背 Qdrant API。
...
**增量更新**：state 里记 \`doc_version\`，upsert 用 doc_id 做幂等键，新版本走 update 节点而非全量重跑。`,
    codeExample: `class IngestState(TypedDict):
    source_type: str
...
graph = builder.compile(checkpointer=PostgresSaver(...))`,
    followupTips: `- **「多路召回怎么接？」** → 入库完成后，问答子图 retrieve 节点并行查 vector + keyword + graph，Send API fan-out，reducer 合并多路结果到 \`docs[]\`。
...
分期落地：第一期单点高频场景 Workflow；第二期要审批和回溯上 LangGraph；第三期评测闭环。不会的业务用模板答——拆功能、画 state、说工具，面试官看思路不考你真用过那软件。`,
    plainTalk: `**一句话**：不熟悉具体系统没关系——先拆功能点，每个点说 AI 怎么辅助，再用 LangGraph 套审批流和工具编排，面试官看的是思路不是业务细节。

**打个比方**：像给陌生公司做 IT 咨询——不需要用过他们的 ERP，但要能问清「谁填什么表单、谁审批、数据从哪来」，然后画一张改造蓝图。`,
    detailAnswer: `编程导航二面经典场景题，考察**系统设计思路**而非特定业务知识。
...
**不熟悉系统的模板答法**：「我会先问三个问题——核心用户是谁、最高频的三个操作是什么、有没有审批节点——然后按上面的框架套，具体 tool schema 接入时再对齐。」`,
    codeExample: `class TaskState(TypedDict):
    task_type: str
    draft: str
...
graph = builder.compile(
    ...
    followupTips: `- **「不熟悉业务系统怎么办？」** → 主动问三个问题（用户/高频操作/审批节点），用通用模板套；强调 tool 层 adapter 隔离业务细节，图编排层不依赖具体系统。
...
LangGraph 优势是失败可定位到具体 node，评测不是只看最终答案。`,
    plainTalk: `**一句话**：评测不是考试分数，而是「离线回归 + 在线监控 + bad case 回流」的双循环，LangGraph 的优势是失败能定位到具体 node。

**打个比方**：像医院体检——不只看「人还活着吗」（最终答案对不对），还要查「哪个器官出问题」（哪个 node 跌分），然后针对性治疗（修 prompt/换模型/改 chunk）。`,
    detailAnswer: `百度面经高频题，考察**有没有线上思维**，不是背评测指标名词。
...
LangGraph 的核心优势：**失败可定位到具体 node**，评测从「答案对不对」升级到「哪一步出了问题」。`,
    codeExample: `# 节点级评测：断言轨迹包含关键 node
def eval_trace(run):
...
if run.status == "failed":
    ...
    followupTips: `- **「准确率还能怎么优化？」** → 四层：数据（难例挖掘/合成）、模型（换强/蒸馏）、系统（结构化输出校验）、RAG（chunk/rerank）；强调先定位失败 node 再优化，不是盲目调 prompt。
...
核心是通信契约和失败隔离，不是背协议 RFC。`,
    plainTalk: `**一句话**：A2A 是 Agent 之间协作的「通信协议」，面试不必装专家——降维成「子图代表远程 Agent，外层管信封、超时、错误隔离」即可。

**打个比方**：A2A 像公司跨部门协作的公文格式——不管里面什么内容，信封上要写「发给谁、什么事、什么时候要回复」，收不到回复有超时处理，一个部门挂了不影响其他部门。`,
    detailAnswer: `百度面经 P2 题，考察**知识边界**——知道概念、能降维到 LangGraph 工程实践即可，不必背 RFC。
...
**工程要点**：超时设置（远程 agent 不能无限等）、错误隔离（远程失败不崩整个父图，写 error 进 state 走 fallback）、幂等（correlation_id 防重复委托）。`,
    codeExample: `# 远程 agent 封装为 LangGraph 节点
async def remote_researcher_node(state, config):
    ...
    followupTips: `- **「和 MCP 区别？」** → MCP 是 tool 层协议（怎么连外部服务），A2A 是 agent 层协议（怎么委托任务）；一个 MCP server 可以被 agent 内部使用，A2A 是 agent 之间的协作。
...
阿里淘天是直接项目拷打，同样要先因果链再技术。让面试官听到你做过取舍，不是背选型。`,
    plainTalk: `**一句话**：项目介绍先讲「用户一句话进来系统怎么运转」，最后才提 LangGraph——框架是承载工具，不是开场白。

**打个比方**：介绍一家餐厅，先说「客人点菜→后厨做什么→怎么上菜→出了问题怎么办」，而不是开场报「我们用了德国烤箱、日本刀、法国调料」。`,
    detailAnswer: `阿里/百度面经 P0 题，考察**项目表达逻辑**，比技术细节更重要。
...
**准备清单**：每个节点一个 data point、一个失败 case、一个优化前后对比。让面试官听到**取舍**，不是背选型。`,
    followupTips: `- **「用户一句话进来第一个写日志在哪？」** → 答 API 网关/中间件层（request_id 生成），不是 LangGraph 节点里；体现你理解全链路，不只会图编排。
...
【替换点】说成你自己科研问答或简历项目：意图三路、质量门控、引用校验环。准备每节点一个 data point，比如 grade 阈值多少、rewrite 最多几次。`,
    plainTalk: `**一句话**：搭 LangGraph 项目有固定五步法——定义 State → 写纯函数节点 → 条件边连接 → compile 绑 checkpointer → API 层暴露 stream。

**打个比方**：像搭乐高——先定底板规格（State schema），再拼每个模块（node），用卡扣决定连接方式（条件边），最后装电池（checkpointer）和遥控器（FastAPI stream）。`,
    detailAnswer: `编程导航一面/二面 P0 原题，**五步法**必须能脱稿背诵，再结合自己项目替换节点名。
...
**替换点**：说成自己的科研问答/简历项目，每个节点准备一个 data point（grade 阈值 0.7、rewrite 最多 3 次、P99 延迟 2.3s）。`,
    codeExample: `builder = StateGraph(AgentState)
...
async def chat(req: ChatRequest):
    ...
    followupTips: `- **「最强节点是哪个？」** → 选有技术深度的节点（如 grade 或 cite_check），讲设计理由 + 评测指标 + 优化过程；避免说「都重要」。
...
和图的关系：节点即 prompt 边界，换节点就换 prompt 策略，比 Chain 里一个大 PromptTemplate 清晰。`,
    plainTalk: `**一句话**：LangGraph 里 prompt 不是一个大模板，而是每个节点有自己的 prompt——静态 system 段管角色，动态段从 state 注入当前步需要的上下文。

**打个比方**：像做菜每道工序有自己的「小纸条」——切配看食材清单，炒菜看火候说明，摆盘看出品标准——而不是一张 5000 字的总菜谱从头看到尾。`,
    detailAnswer: `阿里淘天一面原题，考察**prompt 工程在图编排里的落地方式**。
...
def generate_node(state):
    ...
    followupTips: `- **「和 LangGraph 节点关系？」** → 节点即 prompt 边界；每个 node 有自己的 prompt template，从 state 取当前步需要的字段；换节点 = 换 prompt 策略。
...
准备画三张图：state 字段、节点、条件边，30 秒画完 ReAct 环。`,
    plainTalk: `**一句话**：ReAct 就是「想→做→看→再想」的环——agent 节点调 LLM，有 tool_calls 就去 tool 节点执行，执行完回 agent，直到 LLM 不再调工具。

**打个比方**：像侦探破案——推理（Thought）→ 去现场调查（Action）→ 看调查结果（Observation）→ 再推理，直到破案（END）。`,
    detailAnswer: `ReAct 是 LangGraph 最经典的环，**一面 P0 必考**，必须能在 30 秒内画完拓扑。
...
**面试准备**：白板画三张图——state 字段、三个节点、条件边 + 回边，30 秒完成。`,
    codeExample: `def agent_node(state):
...
builder.add_edge("tools", "agent")  # 回边形成环`,
    followupTips: `- **「和 create_react_agent 区别？」** → 功能等价，但手写图能插 sanitize/compress/fallback 节点，精细控制每一步；create_react_agent 适合快速原型。
...
结合 retry 机制：tool 层 max_retries，图层条件边，两层别重复重试浪费 token。`,
    plainTalk: `**一句话**：tool 出错不能崩图——捕获异常写成结构化 error 进 state，让 agent 节点决定重试、换参数还是走 fallback。

**打个比方**：像快递配送——包裹破损（tool 失败）不能整个物流系统停摆，而是记录问题、换一家快递重发或通知用户自取（fallback）。`,
    detailAnswer: `工具错误处理是**生产级 Agent 的刚需**，考察你有没有线上思维。
...
**fallback 设计**：连续 tool 失败 N 次 → 路由到 fallback 节点 → 返回「暂时无法查询，请稍后重试」+ 人工入口。`,
    codeExample: `def tool_node(state):
    try:
...
def route_after_tool(state):
    ...
    return "agent"`,
    followupTips: `- **「超时和限流区别？」** → 超时是请求没返回（retry 同样请求）；限流是 429（等 retry_after 再试或换 endpoint）；处理策略不同，限流需要 backoff。
...
向老板解释：用图省的是分支胶水和故障恢复成本，不是行数；若算出来维护图比写 if-else 贵，就退回 Workflow。`,
    plainTalk: `**一句话**：过度设计不是道德问题，是可量化问题——三条条件边对应三个真实业务分支吗？checkpoint 有 TTL 吗？新人能看懂 state 吗？

**打个比方**：用大吊车搬一张桌子是过度设计——不是吊车不好，是任务不需要。线性三步流程用 LangGraph 就像用大吊车搬桌子。`,
    detailAnswer: `百度/阿里 P0 连问题，考察**技术判断力和诚实度**——敢不敢说「我可能用重了」。
...
builder.compile(interrupt_before=["publish"])`,
    followupTips: `- **「重构回 Workflow 过吗？」** → 如果有，讲真实 story（「最初上图后发现只有线性三步，维护 State 约定成本 > 收益，退回 LCEL」）；如果没有，说「我会定期用三问自检」。
