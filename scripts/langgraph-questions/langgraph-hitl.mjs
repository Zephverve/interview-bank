...
export default [
  {
    ...
    freq: '4+ 篇面经',
    source: '百度/高德/GitHub',
    conclusion: 'checkpointer 在每个 super-step 后持久化 state 快照；支持 Memory/SQLite/Postgres，配合 thread_id 实现会话恢复。',
    followups: '和数据库事务关系？ · 自定义 checkpointer？',
    keyPoints: ['checkpoint = 可重放的状态机快照', 'compile 时注入 checkpointer', '生产用 PostgresSaver'],
    analogy: '像游戏存档点——不是简单截图，而是能从这个点「读档继续」甚至「换条路重玩」的完整状态快照。',
    plainTalk: `checkpoint 是图执行到某一步之后的「状态快照」，checkpointer 是存这些快照的后端（内存、SQLite、Postgres 都行）。compile 时注入 checkpointer，invoke 时传 thread_id，框架自动存取。它的价值不只是断点续聊，更是崩溃恢复、HITL 挂起几天后 resume、以及开发时的时间旅行调试。百度面经强调：checkpoint 不是简单存档，是可重放的状态机快照。`,
    detailAnswer: `**是什么**：checkpoint 是图执行完一个 super-step 后整个 state 的序列化快照，包含各 channel 的值和执行指针（pending 边等）。checkpointer 是持久化这些快照的存储后端。
...
**踩坑**：什么都往 state 里塞导致 checkpoint 体积膨胀；thread_id 和业务主键混用；恢复时不考虑 pending 边是否重跑。`,
    codeExample: `from langgraph.checkpoint.memory import MemorySaver
...
result = app.invoke({"messages": [HumanMessage("你好")]}, config)`,
    pitfalls: 'state 塞太多导致 checkpoint 膨胀；开发用 MemorySaver 上线忘换 Postgres；恢复时不考虑 pending 边重跑。',
    followupTips: `**「和数据库事务关系？」**——checkpoint 是应用层状态快照，不是 DB 事务；但 checkpointer 后端可以用 Postgres，两者正交。副作用操作仍需业务层幂等。

**「自定义 checkpointer？」**——实现 BaseCheckpointSaver 接口，对接公司内部 KV/对象存储。强调序列化格式和 TTL 策略。`,
    oralAnswer: `checkpoint 是图执行到某 super-step 后的 state 快照，checkpointer 是持久化后端。compile(checkpointer=...) 后，每次 invoke 传 thread_id，框架自动存取。
...
百度面经强调：checkpoint 不是简单存档，是可重放快照——恢复时要讲清哪些 channel 写入、pending 边是否重跑。`,
  },
  {
    ...
    freq: '3+ 篇面经',
    source: '牛客 · 百度',
    conclusion: 'thread_id 是编排会话槽，用于 checkpoint 隔离；业务主键（订单号等）放 state 字段，恢复时用业务键做幂等。',
    followups: '多租户怎么隔离？ · 一个用户多个 thread？',
    keyPoints: ['config.configurable.thread_id', '编排 id ≠ 领域 id', '恢复时业务幂等键独立管理'],
    analogy: 'thread_id 像快递柜的柜号（编排层），订单号是包裹里的收货单（业务层）——取件看柜号，发货对账看订单号，不能混。',
    plainTalk: `thread_id 是 LangGraph 的「会话槽」，传在 invoke config 里，同一 thread_id 会加载历史 checkpoint 实现多轮延续。百度面经建议它和业务主键分开：thread_id 给框架编排用，订单号/任务 id 放 state 字段。恢复时用业务键检查外部副作用是否已执行。多租户可在 thread_id 加 tenant 前缀，一个用户也可以有多个 thread 对应不同任务。`,
    detailAnswer: `**是什么**：thread_id 是 checkpointer 用来隔离不同会话的标识符，传在 config.configurable.thread_id 里。同一 thread_id 的多次 invoke 会加载/追加同一串 checkpoint。
...
def execute_payment(state):
    ...
    pitfalls: '用业务主键当 thread_id 导致会话管理混乱；多租户 id 碰撞；恢复不查业务状态重复副作用。',
    followupTips: `**「多租户怎么隔离？」**——thread_id 加 tenant 前缀 + checkpointer 表分区 + 访问鉴权。绝不跨租户共享 thread。

**「一个用户多个 thread？」**——完全可以，每个独立任务一个 thread。举例「写报告」和「查订单」分开。`,
    oralAnswer: `thread_id 在 invoke config 里传：{"configurable": {"thread_id": "user-123-session-456"}}。同一 thread_id 的调用会加载历史 checkpoint，实现多轮延续。
...
多租户可在 thread_id 加 tenant 前缀，checkpointer 表按 tenant 分区。一个用户可有多 thread 对应不同任务。`,
  },
  {
    ...
    conclusion: 'checkpointer + interrupt_before/after 或 interrupt() 挂起，人工审批后 Command/stream(None) 恢复，可用 update_state 纠偏。',
    followups: '审批拒绝怎么走？ · 外部副作用幂等？',
    keyPoints: ['interrupt 是生产级 Agent 分水岭', 'interrupt_before 防高危操作', 'update_state 可改 state 或强制跳转'],
    analogy: '像飞机起飞前的「最后确认」——自动驾驶到跑道尽头暂停，机长点确认才继续，点拒绝就换方案。',
    plainTalk: `HITL 靠两件事：checkpointer 存快照 + interrupt 挂起执行。编译时设 interrupt_before=["execute"] 或在节点内调 interrupt()，执行到这儿就暂停，把控制权还给应用层，前端展示中间结果等人操作。人批准后 stream(None) 或 Command(resume=...) 继续；也可以 update_state 改错的数据再恢复。这是区分「调过 demo」和「上过线」的分水岭——百度面经会深挖恢复时 pending 边是否重跑、副作用有没有幂等键。`,
    detailAnswer: `**是什么**：Human-in-the-loop（HITL）让人类在 Agent 执行过程中介入审批、修改或终止。LangGraph 通过 interrupt 机制原生支持，不需要自己写轮询或状态机。
...
**踩坑**：没配 checkpointer 导致 interrupt 无法跨请求；恢复后副作用重复执行；审批拒绝没有设计正常路由当异常处理。`,
    codeExample: `from langgraph.types import interrupt, Command
...
app = graph.compile(
    ...
    pitfalls: '没 checkpointer 无法跨请求 interrupt；恢复后副作用重复；审批拒绝当异常而非正常路径。',
    followupTips: `**「审批拒绝怎么走？」**——Command(resume={"approved": False}) 或 Command(goto="apology")，条件边读标志位路由。拒绝是正常业务路径。

**「外部副作用幂等？」**——idempotency_key + executed_actions 列表 + 恢复前查外部系统状态。`,
    oralAnswer: `HITL 靠 checkpointer 存快照 + interrupt 挂起。编译时 interrupt_before=["execute"] 或节点内 interrupt({...}) 暂停，把控制权交还应用层，前端展示中间结果等人操作。
...
百度面经会追问：恢复时 pending 边是否重跑、发邮件扣款有没有幂等键。这是区分玩具和生产的分水岭。`,
  ...
  {
    slug: '038-interrupt-before-after',
    ...
    freq: '2 篇',
    source: 'CSDN 高频题',
    conclusion: 'before：操作发生前拦截（转账、删库）；after：产出后审阅打回（生成报告、写代码）。',
    followups: '能否运行时动态设 interrupt？ · 和 breakpoint 废弃 API？',
    keyPoints: ['before = Guardrails', 'after = Review', 'compile 时或动态配置'],
    analogy: 'before 像安检门（没进去先检查）；after 像老师改作业（写完了审阅打回再交）。',
    plainTalk: `interrupt_before 在节点执行「之前」暂停，适合高危操作——还没删库、还没转账，人先看 plan 再决定让不让执行。interrupt_after 在节点跑「完之后」暂停，适合生成类——报告写完了人改几句再继续。选哪个取决于风险在时间线的哪一侧。LangGraph 1.0 的 interrupt() 函数更灵活，可以运行时按 state 条件动态决定要不要暂停。`,
    detailAnswer: `**interrupt_before**：在指定节点执行前挂起。节点内的逻辑还没跑，state 里是前置节点产出的结果。适合「防患于未然」场景——转账、删库、批量发邮件等不可逆操作，人先看计划再批准执行。
...
def transfer_node(state):
    ...
    pitfalls: 'before/after 选错导致该审的没审；静态列表无法应对动态条件；没设计拒绝路径。',
    followupTips: `**「能否运行时动态设？」**——可以，节点内 interrupt() 按 state 条件动态暂停，比编译时列表灵活。举例金额阈值。

**「和 breakpoint 废弃 API？」**——1.0 用 interrupt/Command 替代，语义更清晰。提到说明跟过新版本。`,
    oralAnswer: `interrupt_before=["execute_node"] 在节点执行前暂停，适合高危操作——还没删库、还没转账，人先看 plan。interrupt_after=["draft_node"] 在节点跑完后暂停，适合生成类——报告写完了人改几句再继续。

选点取决于风险在时间线的哪一侧。金融审批、批量邮件用 before；内容创作、方案生成用 after。LangGraph 1.0 用 interrupt() 函数更灵活，可带上下文给审批 UI。`,
  },
  {
    slug: '039-resume-idempotency',
    ...
    freq: '2+ 篇面经',
    source: '牛客 · 百度',
    conclusion: '恢复前检查副作用是否已执行；工具层幂等键；区分已提交 state 和 pending 边；resume 可能重跑节点需防护。',
    followups: 'pending 边会不会重跑？ · thread 恢复和业务恢复区别？',
    keyPoints: ['外部操作配 idempotency_key', '记录 executed_actions 进 state', 'resume 前查业务主键状态'],
    analogy: '像银行转账的「防重复扣款」——同一笔订单号只能扣一次，不管按了几次确认按钮。',
    plainTalk: `百度面经深挖题。interrupt 恢复时要交代清楚：checkpoint 里哪些 channel 已写入、哪些外部副作用已发生、resume 时 pending 边会不会重跑节点。涉及发邮件、下单、扣款的工具必须配幂等键，恢复前先查是否已成功。state 里可维护 executed_actions 列表。thread_id 恢复是编排层，业务主键恢复是领域层——两层都要答才完整。`,
    detailAnswer: `**核心问题**：interrupt 恢复后，框架可能重跑 pending 边的节点。如果该节点有外部副作用（发邮件、扣款、写数据库），重跑就会导致重复执行。
...
**踩坑**：假设 resume 不会重跑节点导致重复扣款；只用 thread_id 不做业务层检查；executed_actions 没持久化到 checkpoint。`,
    codeExample: `def send_email_node(state):
    ...
    pitfalls: '假设 resume 不重跑节点导致重复副作用；只做 thread 恢复不做业务检查；executed_actions 未持久化。',
    followupTips: `**「pending 边会不会重跑？」**——可能会，工程上假设会重跑，所有副作用节点做幂等。具体行为取决于 interrupt 位置。

**「thread 恢复和业务恢复区别？」**——thread 是编排层加载 checkpoint；业务是用 order_id 查外部系统。两层正交都要答。`,
    oralAnswer: `百度面经深挖题。interrupt 恢复时要交代：哪些 channel 已写入 checkpoint、哪些外部副作用已发生、pending 边 resume 时是否重跑节点。
...
thread_id 恢复是编排层；业务恢复是领域层——两者都要答才完整。`,
  },
  {
    ...
    conclusion: 'checkpoint 分段持久化 + 异步节点 + 后台 worker 轮询 thread 状态，避免 HTTP 长连接超时。',
    followups: '任务跑几天怎么设计？ · 进度怎么给前端？',
    keyPoints: ['每 super-step checkpoint', '异步 ainvoke + 任务队列', 'stream 或 webhook 推进度'],
    analogy: '像外卖订单——下单后拿到订单号（task_id），App 轮询看做到哪一步了，而不是站在厨房门口等两小时。',
    plainTalk: `长任务不能指望一次 HTTP 请求跑完。标准模式是：启动时 invoke 传 thread_id，图按节点 checkpoint；HTTP 立即返回 task_id；客户端轮询 get_state 或订阅 stream 看进度。更重的任务放 Celery/ARQ worker 里 ainvoke，每步 checkpoint 后释放 worker。失败从最后 checkpoint 续跑，不从头来。几天级任务还要考虑 checkpoint TTL 和状态压缩。`,
    detailAnswer: `**问题**：Agent 任务可能跑几分钟到几小时（批量处理、多轮 ReAct、大文档分析），HTTP 长连接会超时，进程崩溃会丢失进度。
...
progress = state.values.get("current_step", "unknown")`,
    pitfalls: 'HTTP 同步等待超时；没 checkpoint 崩溃从头跑；不推进度用户以为卡死。',
    followupTips: `**「任务跑几天怎么设计？」**——checkpoint TTL + state 压缩 + 子图拆分 + durable execution。每天自动 summarization 减 state 体积。

**「进度怎么给前端？」**——get_state 轮询简单；stream/WebSocket 实时体验好。展示 current_step 和百分比。`,
    oralAnswer: `长任务不能指望一次 HTTP 跑完。模式是：启动时 invoke 传 thread_id，图按节点 checkpoint；HTTP 立即返回 task_id；客户端轮询 get_state 或订阅 stream 看进度。
...
LangGraph 1.0 的 durable execution 就是面向这个场景。`,
  },
  {
    ...
    freq: '2 篇',
    source: 'GitHub + 官方文档',
    conclusion: 'graph.stream() 支持 values/updates/messages 等 mode，可 SSE 推节点级或 token 级事件给前端。',
    followups: '多 thread 并发 stream？ · 和 LangChain callback 关系？',
    keyPoints: ['stream_mode 选 values 看完整 state', 'messages 模式推 token', '生产用 WebSocket/SSE'],
    analogy: '像看厨师做菜——stream 让你看到「正在切菜」「正在炒」，而不是干等半小时直接上菜。',
    plainTalk: `用 app.stream(input, config, stream_mode=...) 可以流式拿到执行过程。stream_mode="updates" 看每节点 state 更新；"messages" 推 LLM token 级输出；"values" 看每步完整 state 适合调试。生产环境用 FastAPI 包 SSE 或 WebSocket 推给前端，让用户看到「正在检索」「正在生成」比干等体验好很多。`,
    detailAnswer: `**是什么**：LangGraph 的 stream API 在执行图时逐步 yield 事件，而不是等全部跑完才返回。支持同步 stream() 和异步 astream()。
...
**踩坑**：stream 没处理异常导致前端一直等；混用多种 stream_mode 不清楚各自用途；生产没用 SSE 缓冲导致事件丢失。`,
    codeExample: `from fastapi.responses import StreamingResponse
...
async def chat_stream(q: str, thread_id: str):
    ...
    pitfalls: 'stream 异常未处理前端一直等；stream_mode 混用不清楚用途；SSE 没正确格式化。',
    followupTips: `**「多 thread 并发 stream？」**——各 stream 独立 config/thread_id，checkpointer 后端注意并发性能。

**「和 LangChain callback 关系？」**——messages 模式走 LC callback；updates 是 LangGraph 图级事件。两者可组合。`,
    oralAnswer: `用 app.stream(input, config, stream_mode="updates") 拿每节点 state 更新；stream_mode="messages" 推 LLM token；values 看每步完整 state，调试好用。
...
面试可提 node 级流式让用户看到「正在检索」「正在生成」，比干等最终答案体验好。`,
  },
  {
    ...
    freq: '1 篇',
    source: 'GitHub 100 Questions',
    conclusion: '通过 get_state_history 查看历史 checkpoint，可回滚到任意 super-step 重新 fork 执行，便于复现 bug。',
    followups: '和 LangSmith 关系？ · 生产能开吗？',
    keyPoints: ['每个 checkpoint 有 checkpoint_id', 'update_state 从旧点 fork 新分支', '开发利器，生产慎用随意回滚'],
    analogy: '像 Git 的 checkout 到历史 commit 再开新分支——不改变原来的历史，但从那个点尝试另一条路。',
    plainTalk: `时间旅行就是 get_state_history(thread_config) 列出该 thread 所有历史 checkpoint，选中某个 checkpoint_id 用 update_state 从该点 fork 新执行分支。开发时特别好用：用户报了 bug，回到出错前一步，改 state 或改路由重跑，看「如果当时走了另一条路」会怎样。LangSmith 可视化每步 state 变化，和时间旅行互补。生产环境回滚要谨慎，涉及已发生的副作用。`,
    detailAnswer: `**是什么**：Time Travel 利用 checkpointer 保存的历史 checkpoint，可以查看、回滚、fork 任意 super-step 的执行状态，是从历史点重新执行或修改后重跑的能力。
...
**踩坑**：生产环境随意 fork 导致副作用重复；history 太多不清理占存储；回滚后不更新代码版本导致行为不一致。`,
    codeExample: `config = {"configurable": {"thread_id": "debug-thread-1"}}
...
for event in app.stream(None, old_config):
    print(event)`,
    pitfalls: '生产随意 fork 导致副作用重复；history 不清理占存储；回滚后代码版本不一致。',
    followupTips: `**「和 LangSmith 关系？」**——LangSmith 可视化 trace；Time Travel 是运行时 fork 重跑能力。互补。

**「生产能开吗？」**——查看 history 可以（注意权限）；随意 fork 回滚生产慎用，副作用可能已发生。`,
    oralAnswer: `时间旅行指 get_state_history(thread_config) 列出该 thread 所有历史 checkpoint，可以选中某个 checkpoint_id 用 update_state 从该点 fork 新执行分支，复现「如果当时走了另一条路」的行为。
...
生产环境回滚要谨慎，涉及已发生的副作用；开发环境可随意实验。`,
  },
  {
    ...
    conclusion: '节点内 interrupt() 可按运行时条件动态挂起，比编译时固定 interrupt_before 更灵活。',
    followups: '和静态 interrupt 列表取舍？',
    keyPoints: ['interrupt() 带上下文给审批 UI', '条件满足才暂停', 'LangGraph 1.0 推荐方式'],
    analogy: '像红绿灯感应器——车多时才变红，而不是固定每隔 30 秒就停，不管有没有车。',
    plainTalk: `动态断点用节点内 interrupt(payload) 实现——比如金额超过阈值才 interrupt，小额直接过。比 compile 时写死 interrupt_before 列表灵活得多，审批 UI 还能拿到 payload 里的详情（金额、收款人等）。静态列表适合「这几个节点永远要审」；动态适合「视 state 内容决定要不要人看」。1.0 后 interrupt/Command 是一等公民，面试提到说明跟过新版本。`,
    detailAnswer: `**是什么**：Dynamic Breakpoints 指在节点执行过程中，根据运行时 state 条件动态决定是否 interrupt 挂起，而非编译时固定 interrupt 列表。
...
**踩坑**：interrupt 条件写错导致该审的不审；payload 信息不足审批人无法判断；动态和静态 interrupt 混用逻辑混乱。`,
    codeExample: `from langgraph.types import interrupt
...
def transfer_node(state):
    ...
    pitfalls: 'interrupt 条件写错该审的不审；payload 信息不足；动态和静态混用逻辑混乱。',
    followupTips: `**「和静态 interrupt 列表取舍？」**——静态适合固定合规（永远要审的节点）；动态适合条件触发（金额阈值、风险评分）。可以组合使用。

**补充**：提到 1.0 interrupt/Command 替代旧 breakpoint，说明跟进新版本。`,
    oralAnswer: `动态断点用节点内 interrupt(payload) 实现——比如金额超过阈值才 interrupt，小额直接过。比 compile 时写死 interrupt_before 更灵活，审批 UI 能拿到 payload 里的详情。
