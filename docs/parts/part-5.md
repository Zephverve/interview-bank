...
我没有直接否定，而是做了两件事。第一，用 120 问黄金集跑消融：纯向量 Recall@5 只有 0.51，加 BM25+Sparse 三路混合后跳到 0.63 左右，再加 RRF 重排到 0.71 附近，长尾术语类问题改善最明显。第二，解释互补性——向量语义泛化、BM25/Sparse 精确命中术语，后来 Agentic 层（改写/HyDE/Multi-hop）再把 0.85 推上去。
...
<div class="q-conclusion">💡 <strong>15 秒结论</strong>：精力脉冲分配：Agentic RAG 为主线（2024.12 起）、EvoAgent 在 2026 集中迭代；每周一个唯一重点。</div>
<div class="q-followups">🔁 <strong>追问方向</strong>：同时被导师和实习追着跑怎么办？ · 哪个项目投入最多？</div>
</div>

"诚实地说，不是所有时间都平等分配的。

**垂域科研 RAG** 是读研后的主线：2024.12 起从基线 RAG 迭代到 Agentic RAG（LangGraph、MinerU、120 问黄金集、Recall@5 0.85），持续维护给课题组用。

**EvoAgent** 是 2026.05 起的个人项目，在 RAG 链路相对稳定后集中迭代——把记忆、Conductor、对抗验证、自我进化落成可 demo 的框架。

两个项目时间上有重叠，但阶段精力清晰——我习惯每周一列'本周唯一重点'，其他事情围绕它安排。"
</div>
</details>
...
我读研期间做了两个核心个人项目——**Agentic RAG 科研问答**和 **EvoAgent 框架**。做完之后我清楚了自己的倾向：我喜欢把技术落地成可对话、可溯源、可演进的服务，而不是只在实验室里做 demo。工程训练给我打下了很好的方法论基础，但我更想在大厂的工程环境里成长——真实的用户流量、生产级的系统要求、和不同角色的同事协作，这些是实验室给不了的。
...
但我一直在有意识地补这个短板。FastAPI SSE、连接池、Qdrant 增量索引、意图路由控延迟——都是在实验室尽可能模拟生产。EvoAgent 的 **MixinSession 故障切换**、Conductor 状态持久化、对抗验证，也是按生产级容错思路设计的。
