---
title: Claude Code为什么不用RAG检索代码？Grep、Glob、Read与代…
round: 一面/二面
difficulty: ⭐⭐⭐
tags: [AI编程, 卡码笔记]
point: Claude Code为什么不用RAG检索代码？Grep
source: 卡码笔记
sourceUrl: https://notes.kamacoder.com/interview/llm/claude_code_grep_rag_interview.html
---
...
**结论句（15 秒）**：Claude Code 不用传统 RAG 作为代码检索主路径，不是因为 RAG 落后，而是因为代码检索更需要实时性、精确性、可解释性和多轮决策
...
**结论句（15 秒）**：Anthropic 官方文档里列出的 Claude Code 工具，能看到 Glob、Grep、Read、Task 这些工具
...
**结论句（15 秒）**：&quot;既然有 Bash，为什么还要单独做 Grep 工具
...
**结论句（15 秒）**：如果只说 Claude Code 用 grep，所以不用 RAG，这个答案还不够
...
**结论句（15 秒）**：&quot;Claude Code 为什么不用 RAG
...
**结论句（15 秒）**：- Anthropic Claude Code Settings，工具列表里包含 Glob、Grep、Read、Task 等工具：https://docs.anthropic.com/en/docs/claude-code/settings
