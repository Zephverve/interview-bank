---
title: "Agent 记忆的类型：短期记忆、长期记忆、工作记忆"
round: 一面/二面
difficulty: ⭐⭐
tags: [记忆, AI100]
point: "Memory & State"
source: Agent Interview 100
sourceUrl: https://github.com/BigKunLun/Agent-Interview-100/blob/main/05-memory-and-state/040-memory-types.md
---

**题目**：Agent 记忆的类型：短期记忆、长期记忆、工作记忆

**结论句（15 秒）**：LLM Agent 的记忆系统借鉴认知科学，分为三种核心类型：**短期记忆（Short-Term Memory）**——当前会话的对话历史，存在于 LLM 的上下文窗口中，会话结束即丢失；**长期记忆（Long-Term Memory）**——持久化存储在外部系统（向量数据库、关系数据库）中的知识，跨会话保留，需要检索才能使用；**工作记忆（Working Memory）**——Agent 在当前任务中主动维护的关键信息子集，本质上就是上下文窗口中当前可用的信息。关键理解：LLM 本身是无状态的，所有"记忆"都是工程层面的外部实现。

**追问方向**：误区："上下文窗口越大，就不需要长期记忆了" · 误区："短期记忆和工作记忆是一回事" · 追问："如何决定什么信息值得存入长期记忆？"
