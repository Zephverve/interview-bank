---
title: "混合检索：如何结合语义检索和关键词检索？"
round: 一面/二面
difficulty: ⭐⭐⭐
tags: [RAG, AI100]
point: "RAG"
source: Agent Interview 100
sourceUrl: https://github.com/BigKunLun/Agent-Interview-100/blob/main/02-rag/016-hybrid-retrieval.md
---

**题目**：混合检索：如何结合语义检索和关键词检索？

**结论句（15 秒）**：混合检索（Hybrid Search）并行运行向量语义检索和 BM25 关键词检索，然后通过融合算法（如 Reciprocal Rank Fusion, RRF）将两组结果合并为统一排序列表。语义检索擅长理解意图和同义词匹配，关键词检索擅长精确术语匹配（产品编号、专有名词）。两者互补，实际生产中通常再加一层 Cross-Encoder 重排序形成"两阶段检索"架构。

**追问方向**：误区："语义检索总是比关键词检索好" · 误区："RRF 的 k=60 需要调参" · 追问："如何设置 BM25 和向量检索的权重比？"
