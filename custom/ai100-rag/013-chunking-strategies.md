---
title: "文档分块（Chunking）策略有哪些？各有什么优缺点？"
round: 一面/二面
difficulty: ⭐⭐
tags: [RAG, AI100]
point: "RAG"
source: Agent Interview 100
sourceUrl: https://github.com/BigKunLun/Agent-Interview-100/blob/main/02-rag/013-chunking-strategies.md
---

**题目**：文档分块（Chunking）策略有哪些？各有什么优缺点？

**结论句（15 秒）**：主要有三种分块策略：**固定大小分块**（按字符/token 数切分，简单高效但破坏语义）、**递归分块**（按段落→句子→词层次切分，是最推荐的通用默认方案，Recall 85-90%）、**语义分块**（基于 Embedding 相似度在语义断点处切分，Recall 91-92% 但需要额外 Embedding 开销）。核心权衡是：**上下文保留 vs 检索精度**——块越大保留越多上下文但稀释相关性，块越小匹配越精确但丢失上下文。

**追问方向**：误区："语义分块一定最好" · 误区："块越小越好" · 追问："overlap 有什么用？设多少合适？"
