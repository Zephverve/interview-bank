---
title: RAG落地最难的地方在哪？文档预处理、召回质量、生成忠实度
round: 一面/二面
difficulty: ⭐⭐⭐
tags: [RAG, 卡码笔记]
point: RAG落地最难的地方在哪？文档预处理、召回质量、生成忠实
source: 卡码笔记
sourceUrl: https://notes.kamacoder.com/interview/llm/rag_hardest_parts_interview.html
---
...
**结论句（15 秒）**：先看 RAG 的完整链路（向量检索、Rerank、Chunk 这些基础概念如果还不熟，建议先看 RAG 大厂面试题汇总）：用户提问 → 检索相关文档 → 把文档和问题一起丢给大模型 → 大模型生成回答
...
**结论句（15 秒）**：很多人聊 RAG，直接从 Embedding 和检索开始聊，好像文档天然就是干净的结构化文本
...
**结论句（15 秒）**：文档预处理做好了，召回也对了，是不是就稳了
...
**结论句（15 秒）**：面试官问 RAG 落地最难的地方，不要只说&quot;召回不准&quot;或&quot;模型幻觉&quot;，要展示从全链路视角的系统性理解
