---
title: "如何设计 Agent 间的任务分配与协调？"
round: 一面/二面
difficulty: ⭐⭐⭐
tags: [多Agent, AI100]
point: "Multi-Agent"
source: Agent Interview 100
sourceUrl: https://github.com/BigKunLun/Agent-Interview-100/blob/main/04-multi-agent/034-task-allocation-coordination.md
---

**题目**：如何设计 Agent 间的任务分配与协调？

**结论句（15 秒）**：多 Agent 任务分配与协调的核心挑战是：如何将复杂任务拆分为子任务、分配给合适的 Agent、并确保它们协同工作产出正确结果。主要策略包括：**集中式规划-分散执行**（中央规划器分解任务，Agent 独立执行）、**LLM 作为协调器**（利用 LLM 推理能力做动态任务分配）、**角色定义 + 能力匹配**（根据 Agent 能力描述自动路由）、**动态分配**（基于运行时状态自适应调整）。关键设计原则：明确定义每个 Agent 的角色和能力边界、选择匹配任务结构的通信模式、以及实现健壮的错误处理和降级机制。

**追问方向**：误区："Agent 越多越好" · 误区："用 LLM 做所有协调决策" · 追问："如何处理 Agent 间的目标冲突？"
