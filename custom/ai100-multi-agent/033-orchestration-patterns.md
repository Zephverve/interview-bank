---
title: "Agent 编排模式：Hub-Spoke、Pipeline、Hierarchical"
round: 一面/二面
difficulty: ⭐⭐⭐
tags: [多Agent, AI100]
point: "Multi-Agent"
source: Agent Interview 100
sourceUrl: https://github.com/BigKunLun/Agent-Interview-100/blob/main/04-multi-agent/033-orchestration-patterns.md
---

**题目**：Agent 编排模式：Hub-Spoke、Pipeline、Hierarchical

**结论句（15 秒）**：多 Agent 编排模式决定了 Agent 之间的控制流和协作结构。三种核心模式：**Pipeline（顺序流水线）**——Agent 按预定顺序链式执行，前一个的输出是后一个的输入，适合线性处理流程；**Hub-Spoke（中心辐射）**——一个中心协调 Agent 分配任务给专家 Agent 并汇总结果，适合任务分派场景；**Hierarchical（层级结构）**——多层管理 Agent 逐级分解和委派任务，适合复杂企业级工作流。此外还有 Parallel（扇出/扇入）和 Maker-Checker（循环校验）等补充模式。选择取决于任务的依赖关系、并行度和控制粒度需求。

**追问方向**：误区："选一种模式就够了" · 误区："Hub-Spoke 中协调器必须是 LLM" · 追问："如何在编排模式中加入 Human-in-the-Loop？"
