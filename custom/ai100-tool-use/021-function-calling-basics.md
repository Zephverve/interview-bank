---
title: "什么是 Function Calling？它是如何工作的？"
round: 一面/二面
difficulty: ⭐⭐
tags: [工具, AI100]
point: "Tool Use"
source: Agent Interview 100
sourceUrl: https://github.com/BigKunLun/Agent-Interview-100/blob/main/03-tool-use/021-function-calling-basics.md
---

**题目**：什么是 Function Calling？它是如何工作的？

**结论句（15 秒）**：Function Calling（也叫 Tool Use）是让 LLM 能够调用外部函数/API 的能力。关键理解：LLM 本身不执行任何工具，它只是生成结构化的 JSON 输出，指定要调用哪个函数以及传入什么参数。实际的函数执行由你的应用代码完成，执行结果再返回给 LLM 以生成最终回答。这个能力将 LLM 从"只能说"的顾问升级为"能做事"的执行者。

**追问方向**：误区："LLM 直接执行了工具" · 误区："Function Calling = Agent" · 追问："LLM 怎么知道该调用哪个函数？"
