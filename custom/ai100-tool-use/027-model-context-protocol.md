---
title: "MCP（Model Context Protocol）是什么？它如何标准化工具集成？"
round: 一面/二面
difficulty: ⭐⭐⭐
tags: [工具, AI100]
point: "Tool Use"
source: Agent Interview 100
sourceUrl: https://github.com/BigKunLun/Agent-Interview-100/blob/main/03-tool-use/027-model-context-protocol.md
---

**题目**：MCP（Model Context Protocol）是什么？它如何标准化工具集成？

**结论句（15 秒）**：MCP（Model Context Protocol）是 Anthropic 于 2024 年 11 月推出的开放标准，用于标准化 AI 系统与外部工具、数据源的集成方式。类比"AI 界的 USB-C"——过去每个工具集成都需要定制连接器（N×M 问题），MCP 提供统一协议，让任何工具只需实现一次 MCP Server，就能被任何支持 MCP 的 AI 应用使用。协议基于 JSON-RPC 2.0，包含三个核心角色（Host、Client、Server）和三种核心能力（Tools、Resources、Prompts），支持动态工具发现和实时通知。

**追问方向**：误区："MCP 是 Anthropic 的私有协议" · 误区："MCP 替代了 Function Calling" · 追问："MCP 的安全隐患是什么？"
