---
title: "Reasoning 模型（o1/o3/DeepSeek-R1）vs 标准模型：架构差…"
round: 一面/二面
difficulty: ⭐⭐⭐
tags: [推理, AI100]
point: "Planning & Reasoning"
source: Agent Interview 100
sourceUrl: https://github.com/BigKunLun/Agent-Interview-100/blob/main/06-planning-and-reasoning/055-reasoning-models.md
---

**题目**：Reasoning 模型（o1/o3/DeepSeek-R1）vs 标准模型：架构差异与适用场景

**结论句（15 秒）**：Reasoning 模型（又称 Large Reasoning Models, LRMs）与标准 LLM 的核心区别是**测试时计算扩展（Test-Time Compute Scaling）**：标准模型接收输入后直接生成答案，Reasoning 模型会先进行内部"思考"——生成长链的推理 token（internal chain-of-thought），然后再输出最终答案。这种"先想后答"的能力通过**强化学习（RL）**训练获得，而非单纯的监督学习。代表模型包括 OpenAI o1/o3（闭源，RL 训练的推理链）、DeepSeek-R1（开源，纯 RL 自发现推理策略）和 Claude 的 extended thinking 模式（允许模型在生成最终回复前输出一段可配置预算的内部思考链，用户可通过 API 设置 `budget_tokens` 控制思考深度）。关键权衡：Reasoning 模型在数学、编程、科学推理上显著优于标准模型（o3 在 ARC-AGI 上达 87.5%），但延迟更高（思考时间 10s-60s+）、成本更高（思考 token 计费），且对简单任务过度思考反而降低效率。

**追问方向**：误区："Reasoning 模型就是加了 CoT 的普通模型" · 误区："Reasoning 模型在所有任务上都更好" · 追问："DeepSeek-R1 如何用开源模型实现接近 o1 的效果？"
