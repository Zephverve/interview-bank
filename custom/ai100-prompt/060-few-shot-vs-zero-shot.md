---
title: "Few-Shot vs Zero-Shot Prompting：如何选择？"
round: 一面/二面
difficulty: ⭐⭐
tags: [Prompt, AI100]
point: "Prompt Engineering"
source: Agent Interview 100
sourceUrl: https://github.com/BigKunLun/Agent-Interview-100/blob/main/07-prompt-engineering/060-few-shot-vs-zero-shot.md
---

**题目**：Few-Shot vs Zero-Shot Prompting：如何选择？

**结论句（15 秒）**：**Zero-shot Prompting** 不提供任何示例，直接描述任务让 LLM 执行——依赖模型从预训练中学到的通用能力。**Few-shot Prompting** 在 Prompt 中提供 2-5 个输入输出示例，让模型通过"模仿"理解任务格式和期望。选择原则：**任务简单且模型理解充分 → Zero-shot**（更省 token、更灵活）；**任务有特定格式/逻辑或模型表现不稳定 → Few-shot**（更准确、更一致）。研究表明 Few-shot 在分类和格式化任务上优势显著，但在复杂推理任务上，Few-shot 的优势被 CoT 等推理增强技术压缩。随着模型能力提升（GPT-4、Claude Opus 等），Zero-shot 的能力边界在不断扩大——很多过去需要 Few-shot 的任务现在 Zero-shot 就能做好。

**追问方向**：误区："Few-shot 总是比 Zero-shot 好" · 误区："示例越多越好" · 追问："如何选择 Few-shot 的示例？"
